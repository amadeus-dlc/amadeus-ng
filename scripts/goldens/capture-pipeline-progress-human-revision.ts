...
try{
 ...
 const report=(id:string,result:string,single=false)=>{if(result==="rejected") {const human=spawnSync(process.execPath,[join(root,".claude/hooks/aidlc-record-human-turn.ts")],{cwd:root,env,encoding:"utf8",input:JSON.stringify({hook_event_name:"UserPromptSubmit",session_id:"pipeline-test",prompt:"Request Changes: test revision"})});assert.equal(human.status,0,human.stderr);}
 const args=["report","--stage","reverse-engineering","--result",result,"--user-input",result==="rejected"?"Request Changes":"Approve"];if(result==="rejected")args.push("--reason","test revision");if(single)args.push("--single");const observed=run("aidlc-orchestrate.ts",args);observations.push({id,kind:"report",args,exit:observed.status,stdout:observed.stdout,stderr:observed.stderr});};
 ...
 writeFileSync(handoff,"changed\n");utimesSync(handoff,4102444800,4102444800);next("expired");report("await-expired","awaiting-approval");report("human-backed-rejection","rejected");report("await-revising","awaiting-approval");report("approve-revising","approved");report("revise-revising","revised");
