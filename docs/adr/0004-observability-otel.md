...
4. **トレース境界は「1 ターン = 1 トレース」**（オーナー決定。LLM が 1 ターンの中で aidlc ツール群を駆動した一連の呼び出しが 1 本のトレースになる）。
   ...
   - **縮退規則**: ターンコンテキストが引けないプロセス（セッション外実行、並行セッションで解決不能な場合）は自分をルートとする単独トレースに縮退し、属性で相関する。session_id を受け取らない engine のコンテキスト解決規則（active-directive マーカー経由か、最新ターンへのフォールバックか）は orchestration コンテキスト仕様で確定する。
...
- engine のターンコンテキスト解決規則（並行セッション時のキーイングとフォールバック）— orchestration コンテキスト仕様で確定。
