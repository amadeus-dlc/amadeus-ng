...
**ユースケース**（slice 1 の範囲）: `Next`（読み取り専用）、`Continue`（トークン検証＋再構築＋トランザクショナルなカーソル前進）、`Report`（13 段ガード → 集約コマンド）、`Park` / `Unpark`、`JumpResolve` / `JumpExecute`、`SetAutonomy`、`Recompose`、~~`SingleStageRun`~~ → `RecordSingleStageRun` / `RecordSkeletonStance`（2026-09-04、b47 — どちらも `IntentExecution` のコマンドを打つ通常の書込ユースケース。オーナー裁定 B / A）。slice 2: Bolt 8 動詞、Swarm 3 動詞。
...
- 未着手: ~~skeleton stance / `gate:"unresolved"`~~（2026-09-04、b47 で着手 — `actRecordSkeletonStance` / `actSingleRun` とフレーム不変条件 2 本、v2.4）、per-unit 反復、recompose 残り 4 ガード（slice 2）。`stop_hook.qnt` / `audit_lock.qnt` は次号。
