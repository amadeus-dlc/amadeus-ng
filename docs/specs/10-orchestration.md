...
- **B9**: `HUMAN_TURN` の記録は workspace の事実、`humanActedSinceGate`（同秒 fail-closed を含む導出述語）と昇格可否の政策は本コンテキスト。**`HUMAN_TURN` は外部入力として合成ルートが読む**（追記 2026-09-05、b50）— フック `aidlc-record-human-turn.ts` が監査シャードへ直接追記する一次の事実であり、我々のドメインイベントの投影ではないので、合成ルートが連結バッファを読んで値オブジェクト `HumanTurns` にし、集約のクエリへ引数で渡す。述語のもう一方の材料（直近のゲート解決の時刻）は集約自身の状態である。
- **B10**: ~~レビューレシートの鮮度・凍結述語は **verification 所有のクレートを依存として呼ぶ**。二重実装しない。~~ **stage-1 の射程はレシートの鮮度検査のみ**（オーナー裁定 2026-09-02、#51 = A。凍結検査（レビュー後の改変検出）は後続 intent へ繰り延べる）。**受領証の置き場と鮮度の基準（オーナー裁定 2026-09-04、#73 で質問）**: (A) レビュー受領証（upstream の `REVIEW_COMPLETED`、practices-discovery の `PRACTICES_AFFIRMED` も同型）は **集約 `IntentExecution` のイベント**として取り込む（コマンド `record_review_receipt` → イベント `ReviewReceiptRecorded`。監査行は RMU が投影する。verification の独立クレートは置かず、鮮度の判断は集約のクエリに閉じる — 「判断は集約」の規律）。(i) 鮮度の基準は**順序だけ** — 受領証がそのステージの直近の開始（`STAGE_STARTED` 相当）・差し戻し（`GATE_REJECTED` 相当）より後のイベントであること。成果物ハッシュの照合（指紋）は凍結検査に属し後続 intent へ。実装は b48（#7 キュー 5 の残り、b47 の後）。 **受領証の形（オーナー裁定 2026-09-04、b48 で質問）**: (A) 依頼と判定の**対**で取り込む — `request_review` → `ReviewRequested`、`record_review_verdict` → `ReviewCompleted`（15 変種）。通し番号（ordinal）と上限（budget: advisory 1 / adversarial `reviewer_max_iterations` 既定 2 / 実効 none は 0）の検査も集約が持つ。upstream の `freshReviewReceipts` は依頼と対にならない `REVIEW_COMPLETED` を数えず、判定は終端（READY、または上限到達の NOT-READY。advisory は 1 回で終端）のものだけを受領証とするため、判定だけの取り込みでは鮮度判定が食い違う。試行の区切り（フロア）は開始（advance で立つ）・`GateRejected`・`Jumped`（全ステージ）。`StageRevised` は区切らない。段 12（`PRACTICES_AFFIRMED` + `practices-promote`）は **b49** に分ける（同日裁定 A）。 **昇格受領証の形（オーナー裁定 2026-09-05、b49 で質問）**: (A) メモリ層 2 本（`team.md` / `project.md`）への書込は **RMU が投影する** — 合成ルートがドラフト 2 本と正本 2 本を読んで材料を渡し、集約 `IntentExecution` が受理して昇格内容（5 節の本文・印付き規則行・承認者）をイベント `PracticesAffirmed` に載せ、投影が team.md / project.md・状態ファイルの `Practices Affirmed Timestamp` / `Last Updated`・監査行 `PRACTICES_AFFIRMED` を描く（16 変種）。(A) 昇格に失敗したときの監査行 `PRACTICES_OVERRIDE` は**描かない**（拒否は stderr + exit 1 だけ — b48 の `ERROR_LOGGED` 非描画と同じ扱い、deviations #6）。(A) 兄弟動詞 `practices-event`（`PRACTICES_DISCOVERED` / `PRACTICES_OVERRIDE` / `PRACTICES_SECTION_EMPTY`）は**射程外** — 新しい面 `aidlc-state` は `practices-promote` だけを配線し、他の動詞は not-wired 拒否。試行の区切り（フロア）はレビュー受領証と同じ集合（開始・`GateRejected`・`Jumped` 全ステージ。`StageRevised` は区切らない）。
...
- **コマンド（11）**（`complete_stage` は b42 で撤去 — #85 = A。非ゲート完了は b34 以降実行時到達不能）: `start` / `open_gate` / `approve_gate` / `reject_gate` / `revise_stage` / `skip_stage` / `jump` / `park` / `unpark` / `recompose` / `switch_autonomy`。集約の外からチェックボックスやカーソルを直接書く経路は**型として存在させない**。これは集約のコマンド面であり、upstream のエンジン所有 CLI 動詞 11 個（`set` / `checkbox` / `advance` / `finalize` / `complete-workflow` / `gate-start` / `approve` / `reject` / `revise` / `skip` / `park`）とその外部呼び出し拒否面（§9 S3・§6 I9）とは別の層の話である。
- **ドメインイベント（11）**（`StageCompleted` は b42 で撤去 — オーナー裁定 #85 = A 2026-09-02: b34「誕生 = 初期化完了済み」以降、非ゲート完了は実行時到達不能）: `Started` / `GateOpened` / `GateApproved` / `GateRejected` / `StageRevised` / `StageSkipped` / `Jumped` / `Parked` / `Unparked` / `Recomposed` / `AutonomyModeSet`。封筒は ~~{ id: { intent_id, seq_nr }, schema_version, occurred_at, payload }（C5。2026-08-27 改訂 / ADR-010: intent_id と seq_nr は Domain Primitive WorkflowExecutionEventId にまとまった — 値は同じ 2 つ組である）~~ → **失効（2026-08-29 / ADR-010・Bolt B7）**: 本家 v3.0.0 の `EventEnvelope`（輸送のメタデータ = 集約識別子・通番・発生時刻・型判別子を運ぶ）に置き換わった。~~`WorkflowExecutionEventId`~~ 型は削除し（ファイルごと・106 行）、ドメインイベントは輸送メタデータを一切持たない素の serde 型（本家の語で payload）になった。旧 ~~`schema_version`~~ の後継はジャーナル列 manifest の値 `intent-execution-event/1`（~~`workflow-execution-event/1`~~ — 集約改名に追従 2026-08-30 / Bolt B12。未配布期の改名は no-backward-compatibility による。C5 参照）。封筒を組むのはアダプタ層の Repository である。**ドメインイベントはエンティティ**であり、各変種が `id: IntentExecutionEventId`（採番は集約のコマンド内、UUIDv7）と `aggregate_id: IntentExecutionId` を持つ（オーナー裁定 2026-09-02、b40 — 集約 ID をイベントの `id` に流用しない。`seq_nr` / `occurred_at` は引き続き封筒）。`Started` は**自己完結**で、`aggregate_id`（実行集約の識別子）・`intent_id`・`stages`（解決済み計画の写し — 各ステージの slug / phase / plan_action / conditional / display）を運ぶ（2026-09-02 改訂、b39: 従来は `intent_id` のみで genesis が `&Intent` を要し、自ストリームだけで再生できなかった。`IntentExecution: From<(Started, DateTime<Utc>)>` が唯一の genesis 状態導出で、`start` はそれを通る）。計画の写しを載せるためリプレイに `WorkflowDefinition` も `Intent` も要さない（ADR-008 / C5、Bolt B3 実装、b39 で自己完結化。表示属性の焼き込みは ADR-008 の「定義の詳細を複製しない」への限定的な例外 — 定義全体の複製は引き続き禁止）。upstream の監査行（`STAGE_*` / `GATE_*` / `WORKFLOW_*` / フェーズ境界トリオ / `RECOMPOSED` / `AUTONOMY_MODE_SET`）は、このドメインイベントから ReadModelUpdater（U4）が描く**投影**であって集約の発行物ではない（ADR-003、投影規則は C5）。
...
| `report_dispatch` | ~~ドメインサービス（純関数）~~ → **集約 `IntentExecution` のクエリ（`&self`）**（2026-09-04、b46 — オーナー規律「集約は FSM、導出を独立ドメインサービスに置かない」に揃えた。`next_decision` と同じ形） | (verdict, checkbox, gated, final, moved-on, explicit-stage) → 遷移コマンド列 or 拒否。**対応範囲は段 10 の gate-lifecycle アームと §7.3 forward ディスパッチ表に限る**。段 1〜9・11〜13 は `Report` ユースケースが所有（§3） | verdict → サブコマンド選択は**エンジンが**行い、呼び出し側は選ばない |
| `human_acted_since_gate` | ~~ドメインサービス（純関数）~~ → **集約 `IntentExecution` のクエリ（`&self`）**（2026-09-05、b50 — 材料の 2 分類が確定したことによる。`next_decision` / `report_dispatch` と同じ形） | 自身の状態 `last_gate_resolution_at`（直近のゲート解決の時刻 — `GateApproved` / `GateRejected` / autonomous への `AutonomyModeSet` の適用で刻まれる集約自身の歴史）＋ 引数の値オブジェクト `HumanTurns`（監査台帳の `HUMAN_TURN` 行。フックが書く**外部の入力**で、我々のイベントの投影ではない）→ bool。**同秒は常に fail-closed**（シャードを問わない — 我々の解決時刻はジャーナルの発生時刻であって位置を持たないので、「後だ」と証明できないものは拒否側へ倒す。upstream より少し厳しいだけで緩くはならない） | B9 の導出述語 |
| `jump_resolve` | 集約 `WorkflowExecution` のクエリ（`&self`） | (target, cursor) → `JumpDirection` ＋帰属検証 | `aidlc-jump resolve`（純読取）と `execute`（コミット）の分離 |

**位置づけの注記（2026-08-23）**: `next_decision` と `jump_resolve` は**集約 `WorkflowExecution` のクエリメソッド**（`&self`、書込なし）であり、独立したドメインサービスではない — 状態の所有者の外で判断する Ask 型を避けるため（ADR-002 ④、Bolt B3 実装）。**追記 2026-09-02（b38）**: b26 が `next_decision` を集約から削除しクエリ側ビュー（`ExecutionStateView`）へ移していたが、オーナー裁定（クエリ側は DAO で View を読んで返すだけ・計算結果は RMU が投影）により**本表の所在どおり集約へ復帰**した（`IntentExecution::next_decision(&NextRequest) -> NextDecision` — 計画は `Started` で自己完結するため他集約の引数は不要）。併せて `state_binding`（continue_token の状態束縛）と `WorkflowDefinition::scope_cost`（費用節）も集約のクエリとして置いた。RMU はイベント列から集約を `replay` で起こしてこれらを呼び、リードモデルへ投影する（是正 Bolt 2）。本表の「所在」列がその分類であり、入出力の規範は列を問わず本表が持つ。~~`human_acted_since_gate` は集約に置けない横断の判断なので純関数のドメインサービスのままとする（01 §7.1 原則 2）。~~ → **失効（2026-09-05、b50 / #72、オーナー裁定 A′）**: 材料は 2 種類あって性質が違う。(1) 直近のゲート解決の時刻は集約自身のイベントの帰結なので**集約が状態として持つ**（`coding-rules/cqrs-boundaries.md` 規則 4 — 監査シャードは投影＝遅延するリードモデルであり、コマンド側がそこから最新状態を読むのは禁止パターン）。(2) `HUMAN_TURN` はフックがシャードへ直接書く一次の事実であり我々のイベントの投影ではないので、合成ルートが読んで値オブジェクト `HumanTurns` にし**引数で渡す**（`coding-rules/aggregate-references.md`「判断に要るデータはメソッド引数で渡す」）。したがって述語は横断ではなく**集約のクエリ**であり、昇格の可否（I11）はそのクエリを呼ぶ `switch_autonomy` のガードに閉じる。

## 3. ユースケース層

CLI 動詞・フック応答 1 つ = ユースケース 1 つ。ポート（trait)はこの層で定義する。

**ユースケース**（slice 1 の範囲）: `Next`（読み取り専用）、`Continue`（トークン検証＋再構築＋トランザクショナルなカーソル前進）、`Report`（13 段ガード → 集約コマンド）、`Park` / `Unpark`、`JumpResolve` / `JumpExecute`、`SetAutonomy`、`Recompose`、~~`SingleStageRun`~~ → `RecordSingleStageRun` / `RecordSkeletonStance`（2026-09-04、b47 — どちらも `IntentExecution` のコマンドを打つ通常の書込ユースケース。オーナー裁定 B / A）。slice 2: Bolt 8 動詞、Swarm 3 動詞。 b48（2026-09-04）: `RecordReview`（`RequestReview` / `RecordReviewVerdict` — `aidlc-log review` の書込ユースケース。定義集約のクエリ `review_policy` で `ReviewPolicy` を引き、`IntentExecution` のコマンドを打つ）。`Report` の Approve 段は同じ `ReviewPolicy` を `approve_gate` に渡す（段 11 のレビュアー述語）。 b49（2026-09-05）: `PromotePractices`（`aidlc-state practices-promote` の書込ユースケース。定義は引かない — practices ステージは実行の計画に載っている slug で見つかる。昇格内容の計算は純関数 `PracticesPromotion::plan` であり、それを呼ぶのは合成ルートである）。 b50（2026-09-05）: `SwitchAutonomy`（`aidlc-bolt set-autonomy` の書込ユースケース。定義は引かない — 切替は計画上のどのステージにも紐づかない。入力 VO `AutonomySwitchRequest { mode, turns, human_presence_guard }` は合成ルートが組み、監査台帳の読取（`HumanTurns::find_in`）と env `AIDLC_SKIP_HUMAN_PRESENCE_GUARD` の判定はその外である。昇格の可否は集約 `IntentExecution::switch_autonomy` のガード）。

`Report` は 13 段ガードの実行主体である。ただし段 2 の `--single` は **Controller が `Report` より前に `RecordSingleStageRun` へ分岐**させ、`Report` には到達させない。~~これが I10 の E1（遷移ポート非注入）が成立する条件で~~ → I10 は E4（`SingleStageRunCommitted` の適用がフレーム空であること）で守る（2026-09-04、b47）。state-version guard は `--single` 経路でも段 1 で実施する（turn-shape marker は CP5）。段 11（completion-evidence: pipeline link レシート / per-unit カバレッジ / paused-unit 拒否 / ensemble contribution 証跡）は繰延（slice 2）。**段 12（practices promotion レシート）は b49 で集約のガードへ着地した** — `IntentExecution::approve_gate` の `require_practices_receipt`（段 11 のレビュー受領証ガードの**前**、upstream の段 12 と同じ位置）。approve 側の前提スタック（verifyStageArtifacts / summary confirmation / pipeline link / Practices Affirmed Timestamp / 冪等 replay guard / next-slug 非 SKIP — upstream 03 §5.7）は、レビュアー述語（B10）と同様に verification / workspace への依存として観測する。per-unit カバレッジと paused-unit 拒否の詳細は §7.3〜7.4。
...
| I10 | `--single` は本流を進められない（advance / approve / complete-workflow に**構造的に到達不能**、synthetic workflow id は本流の完了証跡を満たさない） | ~~**E1**~~ → **E4 + 単体**（オーナー裁定 2026-09-04 = B、b47） | ~~`SingleStageRun` ユースケースには遷移ポート自体を注入しない~~ → 隔離実行は `IntentExecution` のイベント `SingleStageRunCommitted` で記録し（新集約は作らない）、その**適用がカーソル・checkbox・`Status`・park・overlay・autonomy・approved を 1 つも動かさない**ことを Quint `engine_loop::single_run_frame`（mutation 検査つき）と集約の単体テストで固定する |
| I11 | autonomous への昇格のみ human presence を要する（降格は不要）。**同秒はシャードを問わず** fail-closed（改訂 2026-09-05、b50 — 我々の解決時刻はジャーナルの発生時刻であって位置を持たないので、同一シャード内の pos 順による tiebreak は使わない。逸脱台帳 #7） | E2+E3 | `AutonomyMode` パーサ（`--mode` の 2 値厳密パース）＋集約のガード `IntentExecution::switch_autonomy`（昇格 ∧ ガード有効 ∧ `!human_acted_since_gate(turns)` → `CommandError::HumanPresenceRequired`）＋単体テスト（述語の 6 形・受理集合）と `Workspace` ハーネスの end-to-end。**Quint はこの述語を持たない** — 材料の `HUMAN_TURN` はフックが書く外部の入力、解決時刻は監査台帳の外の集約状態であり、どちらも `engine_loop.qnt` が抽象化する「エンジンループの核」の外にある（v2.7 ヘッダに記録）。E4 化は旧 `audit_lock`（workspace 第一陣）に合流させる想定だったが、同モデルは ADR-007 で `journal_protocol.qnt`（ジャーナル/スナップショット協定、HUMAN_TURN の同秒判定は対象外）へ改訂されたため未定 — 再検討課題（Bolt B5 設計質問） |
...
| I18 | reviewer 宣言ステージ（実効クラス ≠ none）の承認は**現在の試行の終端受領証**を要する。試行は開始（advance で立つ）・差し戻し・ジャンプ（全ステージ）で区切る。依頼の通し番号は連番・上限内、判定は未回答の依頼にだけ付く | E4+E2 | `engine_loop::{approve_requires_terminal_receipt, review_attempt_floor, review_budget, review_frame}`（v2.5、b48）。実行時は `IntentExecution::approve_gate` / `request_review` / `record_review_verdict` のガード。fingerprint（凍結）・stale-receipt recovery・per-unit・autonomous swarm 例外は繰延（deviations #5） |
| I19 | practices-discovery の承認は**現在の試行の昇格受領証**（`practices-promote` の成功）を要する。試行の区切りは I18 と同じ（開始・差し戻し・ジャンプ）。昇格そのものは本流の状態を 1 つも動かさない | E4+E2 | `engine_loop::{approve_requires_practices_receipt, practices_receipt_floor, promote_frame}`（v2.6、b49）。実行時は `IntentExecution::approve_gate` / `affirm_practices` のガード。`PRACTICES_OVERRIDE` の非描画・`--target-dir` / `practices-event` の未配線は繰延（deviations #6） |
...
- 未着手: ~~skeleton stance / `gate:"unresolved"`~~（2026-09-04、b47 で着手 — `actRecordSkeletonStance` / `actSingleRun` とフレーム不変条件 2 本、v2.4）、レビュー受領証（2026-09-04、b48 で着手 — `actRequestReview` / `actRetryReview` / `actRecordVerdict`、不変条件 4 本 + witness 4 本、v2.5）、昇格受領証（2026-09-05、b49 で着手 — `actPromotePractices` と段 12 のガード、不変条件 3 本 + witness 2 本、v2.6。mutation 3/3）、set-autonomy の受理集合（2026-09-05、b50 で `actSetAutonomy` の `status == Running` を撤去、v2.7 — 新しい不変条件は無く、既存 19 本が green のままであることが検査。I11 の presence ガードはモデル外）、per-unit 反復、recompose 残り 4 ガード（slice 2）。`stop_hook.qnt` / `audit_lock.qnt` は次号。
...
**report の実装ノート（2026-09-04、#73 / b46）**: upstream `handleReport`（`3c3146cf`、`:5464-5927`）の 13 段ガードのうち、状態の値で決まる判断（段 7〜10・段 13・forward ディスパッチ表）は集約のクエリ `IntentExecution::report_dispatch(&self, &Intent, &ReportRequest) -> Result<ReportDecision, ReportRefusal>` が (verdict, checkbox, gated, final, moved-on, explicit-stage) から決め、構文的な段（`--single` / `--skeleton-stance` / `--result` の有無と既知値 / `resume` 分岐 / `--stage` の有無 / 空判定 / env `AIDLC_SKIP_HUMAN_PRESENCE_GUARD`）だけを合成ルートが持つ。逐語はすべてピンから写し、拒否・no-op の材料（slug・checkbox の綴り・execution kind・現在地・scope）は決定と拒否の値が運ぶ（合成ルートは文言のためにリードモデルを読まない）。**分割**: `--single`（`SingleStageRun`、I10）と `--skeleton-stance` は b47 — b46 では構文検証と逐語だけを持ち、本体は「not wired in this build」の `error` directive で止める（b29 の park と同じ層）。**繰延**: 段 11（completion-evidence — slice 2）、段 12（practices-discovery の受領証）、B10 のレシート鮮度（#51 = A）— レシートは upstream では `aidlc-audit append` で外部から足される事実であり、我々の ES でどの集約のイベントとして取り込むかが未裁定（監査追記の動詞もクリティカルパス 4 で未配線）。段 1 の turn-shape marker は Stop フック（クリティカルパス 5）で置き場ごと決める。**到達不能**: 誕生 = 初期化完了済み（b34、#85 = A）以降、非ゲートの `advance` / `complete-workflow`（ゴールデン `report/completed-ungated`）と「approve 済みだが advance 前」（stale re-report guard の recovery 腕）は我々の原子的な遷移では存在しない — `report_dispatch` の表テストで到達不能を固定する。**完了の投影**: ワークフロー完了は RMU がイベント適用後の集約の `Status` から導き、`complete-workflow`（`:2415-2560`）と同じ欄（`Status: Completed` / `In Progress: none` / `Next Stage: none` / `Next Action: Workflow complete` / フェーズ `Verified`）と監査行（承認経路は `approve` が書く `STAGE_COMPLETED` に続けて完了投影が `PHASE_COMPLETED` → `PHASE_VERIFIED` → `WORKFLOW_COMPLETED` の 3 行を足す。読み飛ばし経路は `STAGE_SKIPPED` に続けて同じ 3 行で、`STAGE_COMPLETED` は描かない。`complete-workflow` の `Final stage <Name> completed` 行は upstream `:2498` の再 emit 抑止と同じ理由で report からは到達しない）を描く（b45 で判明した穴の是正。`workflowRollupFields` は cost ledger 由来で我々には無いので鍵ごと省く）。

**`--single` / `--skeleton-stance` の実装ノート（2026-09-04、#73 / b47）**: オーナー裁定（着手前に質問）で**新集約は作らない**（「場当たり的に集約を作らない」）。隔離実行の synthetic-id pair は `IntentExecution` のイベント `SingleStageRunCommitted { stage }` として記録し、RMU が `STAGE_STARTED`（`Stage` / `Agent` / `Workflow: single-stage:<slug>`）→ `STAGE_COMPLETED`（`Stage` / `Details: Single-stage run of <slug> completed` / `Workflow`）の 2 行を監査に投影する（ピン `handleSingleReport` `:5326-5341`。ピンでは `next --single` は何も記録せず、対は `report --single` がまとめて足す — 「開いた試行」の検査は 2.7.x の追加で契約外）。イベントの適用は**フレーム空**（カーソル・checkbox・`Status`・park・overlay・autonomy・approved を動かさない）で、それを I10 の強制手段（E4 `single_run_frame` + 単体）にする。skeleton stance は `record_skeleton_stance` → `SkeletonStanceRecorded { stance }`（受理ガード = 現在地が**静的計画**で Construction フェーズ最初の EXECUTE ステージ — B11 の `first_in_scope_stage_of_phase` と同じ導出、recompose overlay は見ない。再記録は上書き。監査行なし、`Last Updated` も触らない — ピン `set-skeleton-stance`）。`next_decision` は `RunStage.gate` を 3 値 `GateDecision`（`Gated` / `Ungated` / `Unresolved`）で返し、`Unresolved` = ゲート付き ∧ 現在地が skeleton-gate stage ∧ stance 未記録。記録後は `resolveSkeletonGate` がどの stance でも `true` を返すので `Gated`。`read_next_answer.gate` は 3 綴りの 1 列。`next --single` はピン `emitSingleRunStage` どおり state を読まず何も記録せず、`single: true` / `gate: false` / `next_stage` 無しの run-stage を出す。

**レビュー受領証の実装ノート（2026-09-04、b48）**: オーナー裁定（着手前に質問）= 受領証は依頼と判定の**対**で `IntentExecution` のイベント（`ReviewRequested { stage, reviewer, iteration, retry }` / `ReviewCompleted { stage, reviewer, iteration, verdict }`）。レビューの方針（宣言 reviewer・実効クラス = min(宣言, scope `review_cap`, `Review Override`)・上限・per-unit）は定義集約のクエリ `WorkflowDefinition::review_policy(slug, scope, override) -> Result<Option<ReviewPolicy>, UnknownStage>` が値オブジェクトで返し、`IntentExecution` は `Option<&ReviewPolicy>` を引数で受ける（材料は引数、参照は ID の規律）。集約は計画と同じ長さの `review_attempts: Vec<ReviewAttempt>`（`requests` / `pending` / `closed`）を持ち、`request_review`（通し番号 = `requests + 1`・上限内、`--retry-pending` は未回答の番号にだけ受理しフレーム空）と `record_review_verdict`（未回答の番号にだけ受理）で進め、`advance_from` で立った次ステージ・`GateRejected` のステージ・`Jumped`（全ステージ）で空にする（upstream `freshReviewReceipts` のフロア 4 種の写し。`StageRevised` はフロアではない）。approve の段 11（レビュアー述語）は `approve_gate` の FSM ガード — checkbox 前提の後・変更の前で `policy.requires_receipt() && !attempt.has_terminal(policy)` を `ReviewReceiptMissing` で拒む（upstream が `verifyReviewerPrecondition` を `aidlc-state.ts approve` の同じ位置に置くのと同型）。終端判定は `terminalReviewVerdict` の写し（READY、または advisory / 上限到達の NOT-READY）。fingerprint を持たない本 build では非終端 NOT-READY は読み飛ばす（upstream も fingerprint が使えないときは無効化しない）。RMU は監査 2 行（`## Review Requested`: Stage / Reviewer / Iteration / [Retry: pending-request]、`## Review Completed`: Stage / Reviewer / Iteration / Verdict）を描くだけで、状態ファイル・`read_*` 表・`Last Updated` は動かさない。動詞 `aidlc-log review` は新しい面 `Face::Log`（合成ルートは `parseFlags` の写しと逐語だけ。成功は stdout JSON 1 行、失敗は stderr + exit 1、`ERROR_LOGGED` 行は描かない）。Quint v2.5（`reviewed` / `advisory` を静的 nondet、`reqCount` / `pending` / `terminal`、不変条件 4 本 + witness 4 本、ITF 13 本）。**繰延**: Artifact / Source Fingerprint と stale-receipt recovery（凍結検査の intent）、`--unit` / `--single` の受領証（not-wired 拒否、slice 2）、autonomous swarm 例外、unit-major の `STAGE_STARTED` 非フロア、段 12（`PRACTICES_AFFIRMED` / `practices-promote` = **b49**）。

**昇格受領証の実装ノート（2026-09-05、b49）**: オーナー裁定（着手前に質問）= メモリ層 2 本（`team.md` / `project.md`）への書込は **RMU の投影面**である（A）。合成ルート（`aidlc-state practices-promote`）はドラフト 2 本と正本 2 本を読んで昇格内容を純関数 `PracticesPromotion::plan` で計算し、集約 `IntentExecution::affirm_practices` に渡す。集約はそれをイベント `PracticesAffirmed { stage, affirming_user, sections, mandated, forbidden }`（16 変種）に載せ、受領証の 1 bit（`practices_affirmed[stage]`）だけを立てる。投影は upstream の Step 4〜7 と同じ順で 4 面を描く（team.md の 5 節置換 → project.md の印付き規則行の追記（trim 一致で重複除去）→ 状態ファイルの `Practices Affirmed Timestamp` / `Last Updated` → 監査行 `PRACTICES_AFFIRMED` の 4 欄）。ディスクへ落とす順序は upstream の Step 5〜6 どおり **project.md → team.md → 状態ファイル → 監査シャード**であり、メモリ層は**書き替えたときだけ**書く（人が編集する正本でもあるため、触っていないキャッチアップが mtime を動かさない）。段 12 の拒否は `aidlc-state approve` の包み文ではなく **orchestrate 自身の error directive**（stdout・exit 0）である — upstream の段 12 は `aidlc-state approve` を spawn する**前**に効くからである。新しい面 `aidlc-state` は `practices-promote` だけを配線し、他の 24 動詞は not-wired 拒否・未知は upstream `:630` の逐語で断る。

**自律モード切替の実装ノート（2026-09-05、b50 / #72）**: オーナー裁定（着手前に質問）は 2 点。**(A′) human presence ガードの材料は 2 種類あって性質が違う** — (1) 直近のゲート解決の時刻は集約 `IntentExecution` 自身のイベント（`GateApproved` / `GateRejected` / `AutonomyModeSet(autonomous)`）の帰結なので、集約が状態 `last_gate_resolution_at` として自分の歴史から持つ（`coding-rules/cqrs-boundaries.md` 規則 4 — 監査シャードは投影＝遅延するリードモデルであり、コマンド側がそこから最新状態を読むのは禁止パターン）。(2) `HUMAN_TURN` はフック `aidlc-record-human-turn.ts` がシャードへ直接追記する一次の事実で我々のイベントの投影ではないので、b49 のドラフトやメモリ層と同じ**外部の入力**として、合成ルートが `read_all` の連結バッファを読んで値オブジェクト `HumanTurns` にし、集約コマンドへ**引数で渡す**（`coding-rules/aggregate-references.md`）。判断（昇格の可否）は集約のクエリ `human_acted_since_gate` とそれを呼ぶ `switch_autonomy` のガードに閉じる。同一秒のタイはシャードを問わず fail-closed である（upstream は同一シャード内の追記位置で破るが、ジャーナルの発生時刻は位置を持たない — upstream より少し厳しいだけで緩くはならない）。`QUESTION_ANSWERED` / `SUMMARY_CONFIRMATION_RECORDED` は、その動詞がイベントとして配線されるまで解決集合に入れない（逸脱台帳 #7）。**(A) 受理集合は upstream に揃える** — `switch_autonomy` の状態ガード（`NotRunning`）を外し、park 中・完了後・autonomous 中でも切替を受理する（upstream `handleSetAutonomy` は状態ファイルを受理条件として読まない。`--single` / `aidlc-log review` / `aidlc-state practices-promote` と同じ受理集合）。Quint `actSetAutonomy` の `status == Running` も外し（v2.7）、ITF フィクスチャを再採取した。upstream は presence 検査・監査追記・状態書込を 1 つの `withAuditLock` に囲うが、こちらは**1 つのジャーナル追記**がその原子性を担う（ADR-007 のロック退役）— 2 つの付与が同じ turn を消費する競合は楽観 version が弾く。新しい面 `aidlc-bolt` は `set-autonomy` だけを配線し、他の 7 動詞は not-wired 拒否・未知は upstream `aidlc-bolt.ts:908` の逐語で断る。
