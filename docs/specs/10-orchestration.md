...
- **状態（17 属性。~~16 属性~~ → 2026-08-27 改訂 / ADR-010・Bolt B6 で `last_updated_at` を追加）**: `intent_id` / `definition_id` / `definition_revision`（`Started` に記録した定義の系譜 ID と内容版 — ADR-008）/ `stages`（`Vec<StageEntry>` = slug + phase + plan_action + conditional の**文書順**の解決済み計画）/ `plan` / `conditional`（`stages` からの展開列。メメントは別列で持ち、復元時に整合を検査する）/ `overlay`（recompose オーバレイ = 実効プランの源）/ `checkbox` / `cursor` / `status` / `parked_at` / `autonomy` / `approved` / `revision_count` / `seq_nr` / `version`（**楽観 version = ストアが採番する不透明トークン**。集約の遷移では動かず、ドメインは解釈も比較もしない — BR5.3）/ `last_updated_at`（最後に適用したイベントの `occurred_at`。本家 `Aggregate::last_updated_at` の要求で、集約は時計を持たない）。
