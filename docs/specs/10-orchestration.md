...
- 未着手: ~~skeleton stance / `gate:"unresolved"`~~（2026-09-04、b47 で着手 — `actRecordSkeletonStance` / `actSingleRun` とフレーム不変条件 2 本、v2.4）、レビュー受領証（2026-09-04、b48 で着手 — `actRequestReview` / `actRetryReview` / `actRecordVerdict`、不変条件 4 本 + witness 4 本、v2.5）、per-unit 反復、recompose 残り 4 ガード（slice 2）。`stop_hook.qnt` / `audit_lock.qnt` は次号。
