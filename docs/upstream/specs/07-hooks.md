...
**Fail-open as the default failure mode.** Every one of the four PreToolUse guards of §5 treats malformed stdin, a missing state file, an unreadable graph, or any throw as *allow* (`aidlc-state-transition-guard.ts:941-944`; `aidlc-plan-approval-guard.ts:270`, `:284`, `:301`; `aidlc-review-freeze.ts:755`, `:809`; `aidlc-reviewer-scope.ts:734`, `:800`, `:830`). This is a property of those guards, not of every flow-altering hook: the rule-delivery hook of §8.2 fails open on malformed stdin too (`aidlc-deliver-stage-rules.ts:267-271`) but is *not* fail-open on its own two refusal arms — it exits 2 on an unloadable rule bundle and 2/3 on oversize output (§1.1). The three §12a-era guards each also ship a deterministic off-switch: `AIDLC_DISABLE_PLAN_APPROVAL_GUARD=1` (`aidlc-plan-approval-guard.ts:249`), `AIDLC_DISABLE_REVIEWER_SCOPE_HOOK=1` (`aidlc-reviewer-scope.ts:716`), `AIDLC_DISABLE_REVIEW_FREEZE_HOOK=1` (`aidlc-review-freeze.ts:737`).
