...
[read-model-updater-structure.md](read-model-updater-structure.md)。**移行中**（PR1 = 自己診断、PR2 = 参照入力由来の単独面、PR3 = Pipeline 面、
PR4 = 構造化面（`advance_checkpoint` / `prepare_read_model` を取り除き、投影チェックポイントは番号の表の DAO
`ProjectionCheckpointDao` へ移った — 読取の口 `checkpoint` は公開と一緒に PR5 で動かす）。上の表の「投影チェックポイント」は移行前の形の記述）。
...
媒体は実装詳細でポート契約に漏らさない。** **DAO に許されるのはキーによる引当だけ**（追記
2026-09-02、オーナー裁定）: 要求パラメータで行を引く（`WHERE` 相当）のは読取だが、行に無い事実を
導出した時点で CQRS 違反である。読取コマンドが要る答えは RMU が計算結果まで作って
**非正規化リードモデル**（SQLite の `read_*` 表）に置く（[cqrs-boundaries.md](cqrs-boundaries.md)
規則 6 の 2026-09-02 追記）。 どちらを読むかは実装が決めることで、ポート面が
...
例: 旧 `docs/specs/10-orchestration.md`（2026-09-07 に削除）の I8（`next` は読み取り専用）は、`Next` ユースケースに `IntentExecutionRepository` を注入せず、Controller が `find_by_id` 済みの `IntentExecution` を `&` で渡すことで型強制する（設計監査 C2 / 2026-08-23）。
