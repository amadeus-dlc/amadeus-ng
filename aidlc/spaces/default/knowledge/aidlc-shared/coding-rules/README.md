...
| [read-model-updater-structure.md](read-model-updater-structure.md) | **リードモデル更新器は「ジャーナルを読む → 投影 → 表の DAO で更新する」だけ**（2026-09-26 オーナー裁定、Issue #153）。`ReadModelWriter` は作らない。`JournalReader` はジャーナルの要素だけ。DAO は単一テーブルの I/O だけ（1 ファイル = 1 表）。処理したシーケンス番号はリードモデル側の状態で、複数の表を更新するときは表と同じ IMMEDIATE トランザクションで確定してよい（別々なら表が先、番号が後）。ファイルはトランザクションに入れず、追記ファイルはファイルごとの反映済み番号で冪等にする。**移行中**（PR1 = 自己診断、PR2 = 参照入力由来の単独面、PR3 = Pipeline 面、PR4 = 構造化面） | レビュー基準 |
