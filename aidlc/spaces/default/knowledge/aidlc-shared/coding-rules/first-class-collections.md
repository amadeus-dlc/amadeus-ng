...
**イテレータ公開の例外**: 理由付きで可。既存の基本操作・ドメイン固有操作で表せない境界処理に限り、イテレータ公開の理由を記載する。下記のドメイン要素型の規則を緩める例外ではない。

裁定: オーナー、2026-09-06。配列やイテレータを外へ取り出して処理する前に、コレクション自身の操作で表現する。

## 要素はドメイン固有型にする（2026-09-08利用者指示）

ドメインのFCCが扱う要素は、意味と不変条件を持つドメイン固有型とする。`u32`、`usize`、`bool`、`String`、`str`等の生の値を要素として扱わない。借用形でも同じで、`Item = &str` のように外へ出すことで回避しない。プリミティブ型への型エイリアスだけでは、ドメイン固有型への移行にならない。

具体例は `PendingIterations` であり、その要素は `u32` ではなく **`PendingIteration`** とする。保持するコレクション、`FirstClassCollection::Item`、追加・削除・検索・`at`・`filter`・`fold_left` の要素を一貫させる。許容値は現在のドメイン契約から決め、完全コンストラクタで成立させる。

要素の変換は永続化・外部表現の境界で行い、DTOや公開出力の形式は維持する。汎用の `Collection<T>` や `NonEmptyCollection<T>` 自体へ特定のドメインを持ち込むのではなく、ドメイン側で実際に使う要素型を確認する。局所的な作業バッファ、DTO、エラー情報を名前だけでFCCと判定しない。

既存のプリミティブ要素FCCの移行と、その再発を防ぐ `cargo lint` のルール追加は、利用者が許可した後続 [Issue #123](https://github.com/amadeus-dlc/amadeus-ng/issues/123) で扱う。新規コードで違反を増やさない。現時点ではこの要素型規則のリンターは未実装であり、`setter-method` 等の既存ルールの成功を、この規則の検証済みという意味で使わない。
