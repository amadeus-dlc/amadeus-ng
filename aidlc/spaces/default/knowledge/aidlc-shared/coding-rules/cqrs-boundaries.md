...
3. **投影核が要るのはドメインイベントだけ。** RMU は二層でできている（2026-08-28 改訂）:
   ...
   **更新器の内側の構造**（追記 2026-09-26、オーナー裁定 — Issue #153）: 更新器は「ジャーナルを読む → 投影 →
   ...
   **移行中**（PR1 = 自己診断、PR2 = 参照入力由来の単独面（steering・テスト契約・計画指紋・Code Generation 開始可否）、PR3 = Pipeline 面、PR4 = 構造化面（`read_*` 20 表・処理したシーケンス番号・共有面の記録 — `StructuredReadModelUpdater`）。上の「取得ループ（`JournalReader` で差分を引き、チェックポイントを進める）」は
   移行前の形の記述であり、`advance_checkpoint` は PR4 でチェックポイントの表の DAO（`ProjectionCheckpointDao`）と構造化面の更新器へ移った）。
...
6. **クエリ側は RMU が構築したリードモデルだけに依存する。** リードモデルはリード DB 上や
   ...
   **表の形と読み方**（オーナー裁定 2026-09-03 — 基本的な関係モデリング。裁定を仰ぐ前に自分で適用する）:
   主キーは 1 列 `id`、複合主キーにしない。他の列で引くならセカンダリインデックス、自然キーの重複防止は
   UNIQUE インデックス、関連行は FK 列で指す。DAO は **1 表 1 引当**（キーまたはインデックス列の `WHERE`。
   JOIN しない、非正規化の焼き込みもしない）。ユースケースは FK をたどって表ごとに引き、View を組む
   （判断は無い — null の FK は「無し」）。
