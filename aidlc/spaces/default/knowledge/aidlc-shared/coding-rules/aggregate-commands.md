...
**再生方式の訂正（オーナー裁定 2026-09-05）**: 「最新スナップショットとそれ以降の差分イベント集合を使ってリプレイするのが正解」。
同じ集約の最新スナップショットを基底とし、その `seq_nr` より大きいイベントだけを
通番順に適用する。差分が空なら基底の状態が再構成結果になる。以前の「スナップショットは
版・存在検査にのみ使い、ジャーナルを全再生する」という指定は、この裁定で訂正する。
以下の構築 API・memento 型に関する記述を、保存されたスナップショットからの状態復元を
禁止する根拠にしてはならない。
...
- **memento 双子型を作らない** — 集約と構造同一（ID 型の包み紙差だけ等）の「写し」型は複製で
  しかない。この型設計上の制約と、永続化されたスナップショットを再生の基底に使うことは別である。
  再構成では**同じ集約の最新スナップショットと、その通番より後の差分イベント**を使う。
  差分は `event.seq_nr > snapshot.seq_nr` を満たすものを通番順に適用し、基底に反映済みの
  イベントを重複適用しない。
