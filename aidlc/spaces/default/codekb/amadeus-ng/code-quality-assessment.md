...
| TD-2 | 同じ型の DEFERRED 読取後書込が RMU にあと 2 か所。どちらもロック競合を `Corrupt(InvariantViolation)` に写すので、再実行で解ける失敗が「壊れている」に誤分類される | `hook_health_read_model_updater.rs:181`（旧名 `hook_health_reader.rs`、2026-09-25 に型名どおりへ改名）、`workspace_doctor_read_model_updater.rs:183` | 中（link の経路には乗らない） |
