...
rules:
  ...
  - id: BR2.3
    ...
    trigger: "構造化投影時"
    logic: "IF 読取用の行を生成する THEN 集約の規則を参照して結果を投影し、取得ループやDAOに別の業務判断を実装しない。集約が拒否した判断（next_decision の IntentMismatch）は ReadTablesError::IntentUnavailable として投影不能に分類して上へ流し、RMU で判断し直さない（b51）"
  ...
  - id: BR5.3
    ...
    logic: "IF 同じ規約版の有効な共有面のas_ofがtarget未満 THEN 候補の行集合とheadを公開する。同値ならSQLiteの型を含めた行集合の一致をSAVEPOINT内で検査し、一致時だけ共有面を維持してserved_byを記録する。不一致は破損として停止し、比較中のSQL失敗は共有行・head・CPを保持する。targetを超える場合は新しい共有面を維持してserved_byを記録する。欠落・破損はCorrupt(ProjectionSnapshotMismatch)で停止し（候補はpendingのまま保持）、記録済み位置以上の再生成（rebuild_read_model / prepare_read_model）で修復してから候補を再評価する。規約版が古いheadは公開の入口prepare_read_modelが現行版で再生成し、旧規約で保存した計画は再開しない。いずれもheadの世代と内容ダイジェストを書込Tx内で検査する"
