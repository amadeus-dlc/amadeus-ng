...
2026-09-05 の確認に基づき、現行の処理と必要な是正を分けて記述する。PublicationBatch / OutputPlan による復旧は追加設計であり、**未実装**。本書の完成は冪等性の不具合の修正完了を意味しない。
...
5. 全ファイルの反映後、W8 に従って共有面への公開または新しい既存面の維持を決める。その決定、個別確定位置の target_position への更新、served_by、計画の committed を不可分に確定する。失敗時はすべて旧値を保持し、同じ計画から再試行する。
6. 完了を返す。確定した計画を再度受け取っても、ファイルもチェックポイントも変更しない。
...
例: 計画Bが断面120を公開した後に計画A（断面100）が確定する場合、共有面は120のまま、Aの個別位置は100、A.served_byは120の共有世代となる。Aの次回処理が後続行を反映する。共有面の行同士が異なる断面になることや、Aのために120を100へ戻すことは許さない。
...
| 計画後に新しいイベントが到着 | 未完計画の到達位置を変えず完了し、次回に追加分を処理 |
...
入力の不正、投影材料の欠落、計画保存失敗、対象競合、ファイル読書失敗、確定失敗を区別して返す。エラー表示に必要な対象・位置・原因を保持する。最終文言と終了コードのCLI契約は U7 と共有する。
...
## 7. 現行との差、共有契約への影響、検証状況

| 項目 | 現在確認できたこと | 残る作業 |
|---|---|---|
| 取得・計画・構造化面 | read_model_updater.rs は差分の有無を確認し、同じ全履歴読取から両面と前進先を導く。参照規則の同期は別の入口 | 共有 U4 定義の「書込先は互換ファイルのみ」を後続裁定と整合させる |
| 書込と確定 | 現行は規則の管理部分・状態・監査を描いた後、構造化面と位置を進める | PublicationBatch / OutputPlan の保存・照合・排他・再開の契約を追加する。既存ポートで実現済みとは扱わない |
| 冪等性 | 既存取得ループテスト29件成功。一方、同じ出力先を残してチェックポイントを戻した再試行相当では、監査イベント2→4、状態は不変 | BR3.1〜BR3.4 と第5節の障害境界試験を実装する |
| 共有 C5 | 同じ通番を2回描かない規定があるが、チェックポイントだけでは障害の隙間を閉じられない | 公開計画と出力照合を含めた保証へ具体化する。要求を重複許容へ緩めない |
| 共有 C3/C6 | 読取・確定位置のポートは存在するが、ここで追加する公開計画の操作はまだ契約にない | 後続の契約・非機能設計で保存場所、排他範囲、保全期間と整理手順を具体化する。既存の保存先や本家ライブラリの契約を無断で変更しない |
| 旧イベント表 | C5 等には旧イベントID、撤去された完了経路、古いペイロードの記述が残る | 現行イベント・後続裁定・ゴールデンの対応を個別に示す。表全体が今も実装どおりとは主張しない |

この追加設計は、既存出力との互換を守りながら同じ公開操作を再開するためのものである。管理記録の保全期間やI/O中の排他時間などの非機能上の費用は後続で検討する。上記の全停止点で実装が合格したという証拠はまだない。
