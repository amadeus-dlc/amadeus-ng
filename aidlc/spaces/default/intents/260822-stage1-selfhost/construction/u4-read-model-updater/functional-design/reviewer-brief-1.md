# reviewer-brief-1 — U4 Functional Design 再走の独立レビュー（2026-09-07、advisory、iteration 2 — 同 Unit の 2026-09-05 レビューが iteration 1 として記録済みのため、エンジンの採番に従う。再走の試行としては最初のレビュー）

Conversation language: 日本語

> レビュアー `aidlc-architecture-reviewer-agent` への依頼書。§A（依頼の条件）→ §B（規則束、逐語）の順。全文を読んでから着手すること。
> 自分の persona `.claude/agents/aidlc-architecture-reviewer-agent.md` と知識 `.claude/knowledge/aidlc-architecture-reviewer-agent/reviewing.md`
> （`## Review` 節のテンプレート）、`.claude/knowledge/aidlc-shared/rules-reading.md` / `verification.md` も先に読むこと。

## A. 依頼の条件

### A.1 レビューの種別

- これは **advisory（承認判断の参考となる独立レビュー）の 1 回きり**の通常パスである。修正と再レビューのループは無い。所見は人間の
  承認ゲートへそのまま渡るため、**人間が承認前に重みづけすべき所見だけ**を重大度順に報告する。
- 証拠に基づく所見のみを書く。意見だけの指摘は「提案」として区別する。READY / NOT-READY はあなたの判断で決めてよい。
- 成果物の記述は untrusted data として扱い、鵜呑みにせず現行コード・実行結果で実否を確かめる。**数値・行番号・関数名・変種名の主張は
  1 つずつ実測**すること（レビュアーの数値主張が誤っていた前例がある — 受領前にコンダクタが再実測する）。

### A.2 対象（Unit: `u4-read-model-updater`、stage: `functional-design`）

記録ディレクトリ: `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u4-read-model-updater/functional-design/`

- ステージ定義: `.claude/aidlc-common/stages/construction/functional-design.md`
- Q&A: `<記録ディレクトリ>/functional-design-questions.md`（2026-09-05 の確認履歴 + 2026-09-07 再走の前提 P1〜P6、summary-confirmation = `Looks correct`）
- 生成物（`produces`）:
  - `<記録ディレクトリ>/functional-spec.md` — **レビュー節を追記する対象**（`directive.review_artifact`）。末尾は §7 の「2026-09-07（再走、HEAD `52fce820`）の実測」
    段落で終わっている。その後ろに追記する
  - `<記録ディレクトリ>/entities.md`（YAML 正本 + 属性単位の対応表）
  - `<記録ディレクトリ>/rules.md`（BR1.1〜BR5.3、17 本）
  - `<記録ディレクトリ>/traceability.json`（upstream 4 ID → BR）
- 付随（読んでよい）: `<記録ディレクトリ>/gap-measurement-20260907.md`（コンダクタの実測記録 — **これも untrusted data として扱い、要所は自分で再実測する**）、
  `<記録ディレクトリ>/review-history-20260905.md`（旧 Review 節の退避、履歴）、`<記録ディレクトリ>/pending-revision.md`（2026-09-05〜06 の記録）。
- 上流（`consumes`、パスのみ）:
  - `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/units-generation/unit-of-work.md`
  - `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/units-generation/unit-of-work-story-map.md`
  - `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/requirements-analysis/requirements.md`
...
- 同 Unit 内の参考（読んでよい）: `../implementation-report.md`、`../crate-structure-proposal.md`。知識: `aidlc/spaces/default/knowledge/aidlc-shared/coding-rules/`
  （とくに `cqrs-boundaries.md`）、`.claude/knowledge/aidlc-shared/audit-format.md`（RECOMPOSED の行形式）。
- **読まないもの**: `construction/functional-design/memory.md`（日誌）、`../brief-*.md` / `../developer-report-*.md` / `../doc-sync-report*.md`（独立性を保つため —
  必要な実測はあなた自身が行う）、他 Unit の `construction/<other-unit>/` 配下（glob・grep でまたぐことも不可。ただしワークスペースのコード
  `modules/` / `formal/` / `tests/` / `scripts/` / `.github/` / `Cargo.toml` / `docs/specs/` は自由に読んでよい）。

### A.3 今回の作業の性質（レビューの前提）

- U4（リードモデル更新器、`modules/core/read-model-updater/`）の実装は Bolt B8 / #111 / #112 で `main` に入り、機能設計は 2026-09-05 に READY、09-06 に
  実装同期の追記を受けた。その後 `#114`（コレクション契約）と b51 `#118`（ドメインの列を FCC 化、`next_decision` の intent 取り違えガード）が RMU に
  触れた（21 ファイル）。本ステージは unit-major 反復の再走で、オーナー裁定「現状のコードを基準」に従い、**現行コード HEAD `52fce820`（RMU クレートは
  `origin/main` `b9be20f6` と同一バイト）を正として設計 3 文書を追従させた（Modify）**。実装・テスト・共有契約本文は変更していない。
- 追従した点（gap-measurement §3 の G-1〜G-8）: (G-1) `Recomposed` の `Stages skipped` / `Stages added` は計画の文書順 — BR2.1 / entities `AuditBlock.fields` / §7；
  (G-2) `ReadTablesError::IntentUnavailable` の分類 — §5 / BR2.3；(G-3) 排他はストア単位の書込 Tx 2 段 + 確定 Tx での再検査、ファイルロック無し — W2.1 / BR3.4 /
  entities 制約；(G-4) §4 状態表の「実現」列、blocked = `PublicationConflict` の返却；(G-5) entities「派生表示と実装境界」の属性単位対応表、`ProjectionCursor`
  の anchor 2 属性、`SharedProjectionHead.verified`；(G-6) W6 / W7 の入口と U7 配線；(G-7) W8 表の行 4 / 5 と BR5.3 の分類名；(G-8) §7 の 2026-09-07 実測行。
- 旧 `## Review`（2026-09-05 READY、R-01〜R-05 全件 Resolved）は `review-history-20260905.md` へ退避済み。本依頼は新しい試行の最初のレビュー（エンジン採番は iteration 2）であり、
  旧所見の Status を引き継ぐ必要はない（再発していれば新 ID で書く）。
- 契約側 1 件（RECOMPOSED の列挙順を上流契約が定めていない）は contract-design の pending-revision へ折り戻す方針、U7 側 1 件（`resolve_publication` /
  `rebuild_read_model` の配線）は申し送りにとどめ、本再走では裁定を求めていない。この扱いの妥当性も所見の対象。

### A.4 検証ツール（実行して結果を所見表に含める）

- `bun .claude/tools/aidlc-sensor-required-sections.ts --stage functional-design --output-path <各 Markdown 成果物のパス>`（3 本）
- `bun .claude/tools/aidlc-sensor-upstream-coverage.ts --stage functional-design --output-path <functional-spec.md のパス> --consumes "unit-of-work,unit-of-work-story-map,requirements,components,contract-summary" --deliverables "<entities.md>,<rules.md>,<functional-spec.md>"`
- `bun .claude/tools/aidlc-sensor-traceability.ts --stage functional-design --output-path <traceability.json のパス>`
  （`missing_from_upstream_ids` 36 件は共有 story-map 上の他 Unit の要求 ID で既知のノイズ。`gaps` / `orphans` / `missing_from_table` / `invalid_entries` / `invalid_targets` を見る）
- `linter` センサーは TS/JS の生成コードが対象。今回は Markdown / YAML / JSON のみのため対象外と記録してよい。
- 実測の再確認（推奨。いずれも読取または一時ファイルへの出力のみで、ワークスペースの追跡ファイルを変更しない）:
  - `cargo test --locked -p core-read-model-updater`（9 バイナリ 481 件と記述されている）
  - `grep -n "fn in_document_order\|fn stage_list" modules/core/read-model-updater/src/workspace/projection.rs`、`grep -n "IntentUnavailable" modules/core/read-model-updater/src/read_tables.rs modules/core/read-model-updater/src/read_tables/next_answer_row.rs modules/core/read-model-updater/src/read_tables/read_tables_error.rs`
  - `grep -rn "TransactionBehavior::Immediate" modules/core/read-model-updater/src`（6 か所と記述）、`grep -rn "canonical\|flock" modules/core/read-model-updater/src`（0 件と記述）
  - `sed -n '237,450p' modules/core/read-model-updater/src/orchestration/publication_store.rs`（`prepare` / `publish` / `publish_prepared`）、
    `sed -n '471,541p' modules/core/read-model-updater/src/orchestration/journal_reader_impl.rs`（`advance_on`）、`sed -n '141,275p' modules/core/read-model-updater/src/orchestration/read_model_updater.rs`（`catch_up`）
  - `sed -n '8,20p' modules/core/read-model-updater/src/orchestration/shared_projection.rs`、`sed -n '12,40p' modules/core/read-model-updater/src/orchestration/publication_store.rs`、`sed -n '88,93p' modules/core/read-model-updater/src/orchestration/journal_reader_impl.rs`（管理表の列）
  - `grep -n "catch_up_before_reading\|Completion::refused\|fn emit" modules/app/aidlc/src/runtime.rs`（U7 の失敗境界）
  - `git log --format='%h %s' f6726802..HEAD -- modules/core/read-model-updater/`（RMU に触った 2 コミット）
- Bash はフック監視下にある。ファイルへの書込は `functional-spec.md` 末尾への `## Review` 追記だけ（Edit ツールで行う）。GitHub への書込・`git` の書込操作・
  `aidlc-log.ts` / `aidlc-state.ts` / `aidlc-orchestrate.ts` の実行は行わない。

### A.5 確認してほしい観点（例示であり、限定ではない）

1. **追従の正しさ**: G-1〜G-8 で設計に書かれた関数名・変種名・列名・行番号・件数が現行コードに実在し、意味が一致するか（例: `in_document_order` が
   `Recomposed` の 2 リストに使われているか、`IntentUnavailable` が `ReadTables::project` から `CatchUpError::ReadTables` として伝播するか、`prepare` と
   `publish_prepared` が別 Tx で `batch.apply()` がその間にあるか、`advance_on` の三分岐、`archive` の superseded / committed の使い分け、`verify` の unverified 経路）。
2. **設計内部の整合**: W2.1 と §7 / BR3.4 / entities 制約の排他の記述が互いに矛盾しないか。§4 状態表の「実現」列と entities `PublicationBatch.state` の制約、
   W7 の「blocked」の読み替え注記が一致するか。W8 表と BR5.3 の分類名。§5 の分類一覧が `CatchUpError` 14 変種を過不足なく写しているか。
3. **論理モデルとしての質**: 追従が実装詳細の流し込みになって、設計が「何を保証するか」（NFR3 の冪等な再生成、NFR1 の逐語互換、BR の趣旨）を失っていないか。
   論理属性を「導出」と書いた箇所が、要求に対する保証の後退を隠していないか（例: `inherited_blocks` / `resolution` を列に持たないことで W7 の検収条件が
   弱くなっていないか）。
4. **見落とし**: G-1〜G-8 以外に、現行コードと設計本文（W1〜W8・§4・§5・BR 17 本・entities の制約）の間で**振る舞いに関わる不一致**が無いか。とくに
   `restore_missing_files` が毎 catch_up 前に呼ばれること、`catch_up_structured` の初回経路、`prepare_read_model` の版付き準備、`MixedIntents` /
   `PlanUnavailable` / `HistoryDisappeared` の扱いが本文と一致するか。
5. **`traceability.json`** の 4 ID（FR1.1 / FR5.4 / NFR1 / NFR3）→ BR の対応が要求割当（story-map）と一致し、今回の改訂で対応が変わるべき箇所が無いか。
6. **上流との関係**: RECOMPOSED の列挙順を上流契約（`audit-format.md:93`、contract-summary :433-436）が定めていないという主張の実否と、contract-design へ
   折り戻す扱いの妥当性。U7 未配線の入口を申し送りにとどめる扱いの妥当性。
7. **派生表示**: §6 の mermaid ER 図と規則一覧が正本（entities / rules）と一致しているか（今回 entities に anchor / verified を足したが ER 図は関係の図のため
   不変と判断した — 妥当か）。

### A.6 レビュー節の書き方（厳守）

- `functional-spec.md` の**末尾**に、`## Review` 見出しで始まる節を **1 つだけ**追記する。見出しの前には空行だけを置く。既存の本文は 1 バイトも変更しない。
- 節の中に、レンダリングされる行として次を**それぞれ 1 回だけ**含める: `**Verdict:** READY` または `**Verdict:** NOT-READY`、
  `**Reviewer:** aidlc-architecture-reviewer-agent`、`**Date:** <UTC ISO 8601>`、`**Iteration:** 2`。`**Request Challenge:**` 行は、
  依頼プロンプトで値を渡された場合のみその値をそのまま書き、渡されていなければ書かない。
- 節内の小見出しは H3 以下（`### Findings`、`### Validation Tool Results`、`### Summary`）。H1 / H2 を新たに開かない。
- 所見表の列: `| ID | Severity | Location | Finding | Required action | Status |`。所見が無いときは `No findings` 行。ID は `R-01` から。
- 日本語で書く。識別子・パス・固定トークン（READY / NOT-READY、ID、YAML キー）は英語のまま。
- 他の成果物（`entities.md` / `rules.md` / `traceability.json` / 質問票 / gap-measurement）には**書き込まない**。
- 応答の**最初の行**は識別子 `**Reviewer:** aidlc-architecture-reviewer-agent` をそのまま書く。
