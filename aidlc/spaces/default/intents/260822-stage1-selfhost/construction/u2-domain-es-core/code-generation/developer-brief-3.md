# developer-brief-3 — 委任 1: FCC 11 型の新設（U2、Bolt b51、Step 0〜1）
...
- **役割**: `aidlc-developer-agent`（実装担当）。Unit `u2-domain-es-core`、ステージ code-generation、承認済み計画
  `code-generation-plan.md`（指紋 `sha256:dd1170c1a75b16e30a351f34d9f4ff57164bcbe65482361e94e6909de7f0634d`）の **Step 0〜Step 1** だけを実施する。
  Step 2 以降（集約・イベント・境界の切替）は次の委任が行う。**既存の公開 API・既存テストは 1 行も変えない**（追加のみ）。
- **範囲（Step 1）**: `modules/core/command/domain/src/orchestration/` に `stage_entries` / `stage_slot` / `stage_slots` /
  `stage_slots_error` / `stage_index_set` / `artifact_paths` / `stage_slug_set` / `transition_steps` / `transition_steps_error` /
  `review_closures` / `pending_iterations` の private mod を新設し、`modules/core/command/domain/src/workspace/` に `promoted_sections` /
  `promoted_sections_error` / `rule_lines` を新設する。各型は計画 §2 の表（要素・不変条件・操作・`Filtered`・エラー型）に従う。
  共通契約 `core_infrastructure::collections::FirstClassCollection` を実装し、`modules/core/command/domain/tests/collection_contract_test.rs`
  の `check(..)` に登録する（空を許す型は空と非空の 2 例、非空型は非空例のみ）。ファサード `orchestration/mod.rs` / `workspace/mod.rs` に
  `pub use` を追加する（`PendingIterations` は `pub(crate)` でファサード非公開。private mod の `pub(crate)` 型は
  `super::pending_iterations::PendingIterations` で兄弟 mod から参照できる）。
- **所有ファイル（書込可）**: 上記の新設ファイルと両 `mod.rs`（`mod` 宣言と `pub use` の追加行のみ）、`tests/collection_contract_test.rs`
  （登録の追記のみ）。報告 `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/code-generation/developer-report-3.md`。
  **それ以外は触らない**（`intent_execution.rs` / `intent.rs` / イベント型 / `practices_promotion.rs` / `review_attempt.rs` / 兄弟クレート /
  `Cargo.toml` / `Cargo.lock` / `scripts/**` / `.github/**` / `formal/**` / 計画ファイル）。
- **TDD（Testing Contract、tdd / standard）**: 型ごとに Red（`#[cfg(test)]` の失敗テストを先に書き、失敗するコマンドの出力を報告に記録）→
  Green（最小実装）→ Refactor（緑のまま整理）。Red の出力は「コンパイルエラー」でもよいが、その場合はテスト本体を書いた後に
  `cargo test -p core-command-domain --lib <mod>` を実行した実際の出力（先頭数行）を報告に貼る。集合型（`StageIndexSet` / `StageSlugSet`）は
  proptest で Monoid 則（結合法則・左右単位元・冪等・交換）と差集合則（`A \ A = ∅`、`A \ ∅ = A`、`(A ∪ B) \ B ⊆ A`）を固定する
  （`PROPTEST_RNG_SEED=20260823` 付きで実行）。各コンポーネント 5〜8 本を目安にし、正常系 + 異常系 2 件以上。
- **設計の細則**（計画 §2 の補足、迷ったらこちら）:
  - `StageEntries::new(Vec<StageEntry>) -> Result<StageEntries, PlanError>` は現行 `StageEntry::check_plan` の検査（`Empty` /
    `InitializationMustExecute` / `InitializationMustBeUnconditional` / `DuplicateSlug`）を**同じ順序・同じ変種**で行う（`check_plan` 自体は
    次の委任が削除するので、今回は呼び出して再利用してよい）。`at(StageIndex) -> Option<&StageEntry>`、`position_of(&StageSlug) -> Option<StageIndex>`、
    `first_of(PhaseId, PlanAction) -> Option<StageIndex>`、`slugs_at(&StageIndexSet) -> StageSlugSet`、`fold_left`、`filter -> Collection<StageEntry>`。
  - `StageSlot` は値の組（`Clone` + `PartialEq` + `Eq` + `Debug`）。`genesis(key: StageKey, plan_action: PlanAction)` は checkbox = Pending・
    approved = false・revision_count = 0・review_attempt = `ReviewAttempt::default()`（無ければ既存の空コンストラクタ）・practices_affirmed = false。
    `new(全属性)` は DTO 境界用。コマンドは `&mut self` で戻り値なし（CQS）: `mark(CheckboxState)` / `record_approval()` /
    `invalidate_approval()` / `bump_revision()` / `override_plan(PlanAction)` / `reset_attempt()`（会計 reset + affirmed = false）/
    `record_review_request(u32)` / `record_review_verdict(u32, ReviewVerdict)` / `affirm_practices()`。`ReviewAttempt` の既存メソッド
    （`record_request` / `record_verdict` / `reset`）へ委譲する。
  - `StageSlots::new(Vec<StageSlot>) -> Result<_, StageSlotsError>`（`Empty` / `DuplicateSlug { slug }`）、`genesis(&StageEntries) -> StageSlots`
    （各位置を `StageSlot::genesis(StageKey::new(slug, phase), plan_action)` で。`StageKey` の構築は既存 API に合わせる）、
    `at(StageIndex) -> Option<&StageSlot>`、`stage_key(StageIndex) -> Option<&StageKey>`、`position_of(&StageSlug)`、`fold_left`、
    `filter -> Collection<StageSlot>`。位置指定コマンドは `fn mark(&mut self, stage: StageIndex, state: CheckboxState) -> Result<(), StageSlotsError>`
    の形（範囲外は `StageSlotsError::OutOfRange { stage }`）で `StageSlot` のコマンドを 1 対 1 に持ち、一括コマンド
    `mark_all(&StageIndexSet, CheckboxState)` / `invalidate_approvals(&StageIndexSet)` / `reset_attempts_all()` を持つ。
  - `StageIndexSet`: `BTreeSet<StageIndex>` を包む。`empty()` / `singleton(StageIndex)` / `range(from: StageIndex, to_exclusive: StageIndex)` /
    `from_iter` 相当の `new(impl IntoIterator<Item = StageIndex>)` / `contains` / `combine` / `divide` / `at`（昇順）/ `fold_left` / `filter -> Self`。
    `Default` = 空。`StageIndex` の構築は既存の `StageIndex::new`（`pub(crate)`）を使う。
  - `StageSlugSet`: `BTreeSet<StageSlug>`（辞書順）。API は `StageIndexSet` と同型（`range` は無し）。
  - `ArtifactPaths` / `RuleLines`: `Vec<String>` を包む素通し（順序・重複保持）。`empty()` / `new(Vec<String>)` / `at` / `fold_left` / `filter -> Self`。
  - `TransitionSteps`: `new(Vec<TransitionStep>) -> Result<_, TransitionStepsError::Duplicate { step }>`、`single(TransitionStep)`、`contains`、
    `is_single(TransitionStep) -> bool`、`is_pair(TransitionStep, TransitionStep) -> bool`、`at` / `fold_left` / `filter -> Self`。
    `TransitionStep` が `Copy` / `PartialEq` を持つかは実装を確認して合わせる。
  - `ReviewClosures`: `empty()` / `new(Vec<ReviewClosure>)` / `record(ReviewClosure)`（コマンド）/ `has_terminal(&ReviewPolicy) -> bool`
    （現行 `ReviewAttempt::has_terminal` の判定を写す — 今回は写すだけで `ReviewAttempt` は変えない）/ `at` / `fold_left` / `filter -> Self`。
  - `PendingIterations`（`pub(crate)`）: `empty()` / `with(u32)` / `without(u32)`（コマンド、`&mut self`）/ `contains` / `at` / `fold_left` / `filter -> Self`。
  - `PromotedSections::new(Vec<PromotedSection>) -> Result<_, PromotedSectionsError::DuplicateHeading { heading }>`、`at` / `fold_left` /
    `filter -> Self`、`headings(&self) -> Collection<String>`（`fold_left` で組む）。
  - エラー型は手実装 enum + `fmt::Display`（材料のみ）+ `std::error::Error`（thiserror / anyhow 不使用）。`Display` の文言は既存のエラー型
    （例: `orchestration/plan_error.rs`）の様式に合わせる。
  - `FirstClassCollection` の関連型 `Item<'a>` は既存 7 型（`workspace/bolt_refs.rs` / `checkboxes.rs` / `audit_fields.rs`、
    `workflow_definition/stage_graph.rs`、`core_infrastructure::collections::{Collection, NonEmptyCollection}`）の実装を読んで同じ形にする。
  - `#[must_use]`、`missing_docs`（全公開項目に日本語 doc）、`unreachable_pub`、`unwrap` / `expect` 禁止（プロダクトコード）、
    `indexing_slicing` 等の workspace lints 50 本を deny のまま通す。`clippy.toml` によりテストコードでは `unwrap` / `expect` 可。
  - 名前はユビキタス言語（`set_*` / `get_*` / `data` / `helper` / `util` を使わない）。フィールドは private + アクセサ。
- **Step 0（着手前の基線、報告に記録）**: `git status --short` が空（intent 記録の未コミット変更があればそのまま触らない）、
  `git log --oneline origin/main..HEAD`（記録コミットのみ）。次を実測して件数を記録: `PROPTEST_RNG_SEED=20260823 cargo test -p core-command-domain --lib`、
  `cargo test -p core-command-domain --test collection_contract_test`、`cargo test -p core-command-domain --test engine_loop_conformance`、
  `PROPTEST_RNG_SEED=20260823 cargo llvm-cov --package core-command-domain --summary-only`（行 / 関数 / リージョン）と同コマンドに
  `--ignore-filename-regex 'modules/core/command/domain/src/(workflow_definition|workspace)/'` を付けた orchestration 単独値、
  `rg -n '# Panics' modules/core/command/domain/src`、`rg -n 'pub fn .*-> &\[' modules/core/command/domain/src/orchestration modules/core/command/domain/src/workspace`。
- **受入（Step 1 の完了条件、すべて報告に実行結果を貼る）**:
  1. `PROPTEST_RNG_SEED=20260823 cargo test -p core-command-domain` 全緑（lib + ITF + 契約試験）。新設テストの件数と Step 0 からの増分。
  2. `cargo fmt --all --check` / `cargo clippy --workspace --all-targets -- -D warnings` / `cargo lint` 緑。
  3. `git diff --stat origin/main..HEAD -- modules` が `modules/core/command/domain/` に閉じ、`Cargo.toml` / `Cargo.lock` 不変。
  4. `PROPTEST_RNG_SEED=20260823 cargo llvm-cov --package core-command-domain --summary-only` の行カバレッジが Step 0 の基線を下回らない。
  5. コミット 1 つ: `feat(domain): FCC 11 型を新設し契約試験へ登録`（`git add` は `modules/core/command/domain/` に限り、`aidlc/` の記録は
     コミットしない — 記録はコンダクタが回収する）。push はしない。
- **報告（`developer-report-3.md`、日本語）**: Step 0 の基線表、型ごとの Red の失敗出力（先頭数行）と Green / Refactor の要点、受入 1〜5 の
  実行結果、計画 §2 からの逸脱（あれば理由）、次の委任（切替）への申し送り（例: `StageSlot::genesis` の `StageKey` 構築方法、
  `ReviewAttempt::default` の有無、`TransitionStep` の trait）、設計判断が要る問題（実測付き）。計画ファイルは編集しない。
- **禁止**: 既存 API の変更・削除、`aidlc/**` の記録（報告ファイル以外）の編集、`scripts/**` / `.github/**` / `formal/**` / `Cargo.toml` の変更、
  push / PR / GitHub への書込、他者の変更の巻き戻し、`AIDLC_*` 環境変数によるフックの回避。
- **最初に読むもの（順に）**: `aidlc/spaces/default/knowledge/aidlc-shared/coding-rules/README.md` と同ディレクトリの
  `first-class-collections.md` / `module-visibility.md` / `field-visibility.md` / `error-handling.md` / `domain-equality.md` /
  `command-query-separation.md` / `interior-mutability.md` / `ubiquitous-language.md` / `domain-object-kinds.md` / `tell-dont-ask.md`、
  本ファイルの §B（承認済み計画）と §C（テスト指示）、`modules/core/infrastructure/src/collections/*.rs`、既存 FCC の実装例
  `modules/core/command/domain/src/workspace/{bolt_refs,checkboxes,audit_fields}.rs`、`modules/core/command/domain/tests/collection_contract_test.rs`、
  切替対象の現行実装 `modules/core/command/domain/src/orchestration/{intent_execution,intent,stage_entry,stage_key,stage_index,review_attempt,review_closure,transition_step}.rs`
  と `workspace/{practices_promotion,promoted_section}.rs`（今回は読むだけ）、設計 `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/functional-design/entities.md`
  と `rules.md`（BR5.5）、`.../nfr-design/logical-components.md`（§1）。
