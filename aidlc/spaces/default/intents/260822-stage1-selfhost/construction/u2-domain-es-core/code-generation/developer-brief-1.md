...
# developer-brief-1 — U2 ドメイン ES コア（Bolt B3 / 委任 1: workflow_definition 側 + Repository 側 = 計画 Step 1〜8）

あなたは aidlc-developer-agent。承認済み計画 `code-generation-plan.md`（下に全文）と `unit-test-instructions.md`（下に全文）の
**Step 1〜Step 8 だけ**を実装する。Step 9 以降（orchestration 側の ES 化）は委任 2 が行う — 触らない。
埋め込みの Testing Contract（tdd / standard / brownfield）が Part 2 の権威であり、メモリを再解釈しない。TDD: 各 Red で失敗する
コマンド出力（失敗テスト名・要約行）を報告ファイルに記録してから Green に進む。

## 作業場所・ブランチ・コミット
- ワークスペースルート `/Users/j5ik2o/orca/workspaces/amadeus-ng/docs`、ブランチ `bolt/b3-u2-domain-es-core`（作成済み、origin/main 起点）。
  `git push` / PR 作成はしない。コミットは意味単位で行う（例: `refactor(workflow-definition): move PlanAction out of orchestration (FR8.3)`、
  `feat(workflow-definition): add WorkflowDefinitionId / DefinitionRevision (ADR-008)`、`feat(use-case): WorkflowDefinitionRepository::find_by_id`、
  `feat(interface-adapter): derive definition id / revision in WorkflowDefinitionRepositoryImpl`、`test(...)`）。`aidlc/` 配下は**コミットしない**
  （コンダクタが行う）。`git add -A` は使わず、変更ファイルを明示して add する。
- 計画ファイル（`code-generation-plan.md` / `unit-test-instructions.md` / `code-generation-questions.md`）は**絶対に編集しない**（承認バイトが
  凍結されており、書き換えると承認ガードが全体を止める）。進捗・Red 記録・逸脱・棚卸しは
  `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/code-generation/developer-report-1.md` に書く（新規作成、
  Write ツールで）。

## 所有するファイル（委任 1 の書込範囲）
- `modules/core/domain/src/workflow_definition/**`（`plan_action.rs` の移動先、`workflow_definition_id.rs` / `definition_revision.rs` 新設、
  `workflow_definition.rs` の改訂、`mod.rs` の `pub use`）
- `modules/core/domain/src/orchestration/mod.rs`（`mod plan_action` / `pub use plan_action::PlanAction` の削除のみ）と
  `modules/core/domain/src/orchestration/plan_action.rs`（削除 = 移動元）、`modules/core/domain/src/orchestration/workflow_execution.rs`
  （`use super::plan_action::PlanAction` → `use crate::workflow_definition::PlanAction` の 1 行だけ — 本体の ES 化は委任 2）
- `modules/core/domain/tests/engine_loop_conformance.rs`（`PlanAction` の import パスだけ。本体の書き換えは委任 2）
- `modules/core/use-case/src/orchestration/workflow_definition_repository.rs`（trait / GraphReadError）と同 `mod.rs`
- `modules/core/interface-adapter/src/orchestration/workflow_definition_repository_impl.rs`、`.../orchestration/memory/workflow_definition_repository.rs`、
  同 `mod.rs` 群、`modules/core/interface-adapter/tests/workflow_definition_repository_impl_test.rs`、`.../tests/golden_parity_test.rs`
- `tests/golden/upstream-3c3146cf/harness.json`（新規 — upstream 実バイト）と `tests/golden/upstream-3c3146cf/README.md`（表に 1 行追加、既存行のバイトは不変）
- `modules/core/interface-adapter/Cargo.toml`（`canon-json` は既に依存に入っている — 追加不要のはず。確認のみ）
上記以外（特に `orchestration/` の他ファイル、`Cargo.toml` の workspace 設定、CI、`aidlc/`）は触らない。

## 受入基準（委任 1 の Done）
1. `grep -rnE 'enum PlanAction\|pub use .*PlanAction' modules/core/domain/src/orchestration` が 0 件（`workflow_execution.rs` の `use` は対象外）。
2. `WorkflowDefinition::effective_plan_action` / `next_in_scope_stage` が存在しない（grep 0 件）、残す述語 6 つ（`is_valid_scope` / `valid_scopes` /
   `scope_metadata` / `subgraph_for_scope` / `stages_in_scope` / `first_in_scope_stage_of_phase`）と `grid()` 照会は健在。
3. `WorkflowDefinitionId`（非空、trim、`parse` 往復、Eq/Ord/Hash/Display）、`DefinitionRevision`（`sha256:` + 16 進 64 桁のみ受理、Display）が
   `core_domain::workflow_definition` から公開され、`WorkflowDefinition::new(id, revision, graph, grid, scopes)` と `id()` / `revision()` がある。
4. `core-use-case`: `trait WorkflowDefinitionRepository { fn find_by_id(&self, id: &WorkflowDefinitionId) -> Result<WorkflowDefinition, GraphReadError>; }`
   のみ（`find()` は削除）。`GraphReadError` に `NotFound { expected: WorkflowDefinitionId, actual: WorkflowDefinitionId }`（expected = この
   Repository が提供できる id、actual = 要求された id）と `HarnessIdentity { path: String, cause: String }`（`harness.json` の欠落・不正 JSON・
   `name` 欠落）を追加。rustdoc は材料のみ（文言はアダプタ層）。
5. `WorkflowDefinitionRepositoryImpl::find_by_id`: `<data_dir>/harness.json` を読み `name` → `WorkflowDefinitionId`（要求 id と一致しなければ
   `NotFound`、一致すれば 3 入力を読み）、`DefinitionRevision` = `canon_json::hash_canonical(&to_value(&RevisionInput{ stage_graph, scope_grid, scopes }))`
   の `rendered()`（`sha256:<hex>`）— `RevisionInput` はアダプタ層のワイヤ構造体（`stage_graph` = stage-graph.json をそのまま読んだ `serde_json::Value`、
   `scope_grid` = scope-grid.json の値（欠損時は導出グリッドを `{ <scope>: { stages: {...} } }` 形に直列化）、`scopes` = identity frontmatter を
   `name` 昇順に並べた配列。各要素は `name` / `depth` / `description` / `keywords` など frontmatter の値）。同一入力で 2 回 load して一致、
   入力を 1 文字変えれば不一致。失敗態度（グリッド欠損は fatal にしない等）は既存のまま。
6. `InMemoryWorkflowDefinitionRepository::new(definition)` は定義の `id()` と一致する要求だけ Ok、不一致は `NotFound`。
7. `tests/golden/upstream-3c3146cf/harness.json` は upstream ピンの実バイト（`curl -s https://raw.githubusercontent.com/awslabs/aidlc-workflows/3c3146cfd7cef33020d48e8d48d4e80d0f8c2820/dist/claude/.claude/tools/data/harness.json`
   で取得 — 取得不能なら `.claude/tools/data/harness.json` と同一内容であることを README に注記して使う）。README の表に行を追加（bytes / sha256）。
   golden parity test は `find_by_id(&WorkflowDefinitionId::parse("claude")?)` で実グラフを読む。
8. テスト: 計画 §5.1 の Red → Green → Refactor を守り、各コンポーネント 5〜8 本。`find()` の既存呼出 13 箇所を `find_by_id` へ。
9. 品質ゲート緑: `cargo fmt --all --check`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo lint`、`cargo test --workspace`
   （ITF 準拠テストは import パス修正後も緑のまま — 旧 API を使い続けてよい、委任 2 で書き換える）。
10. 報告ファイル `developer-report-1.md` に: 変更ファイル一覧、各 Red の失敗出力（コマンド + 失敗テスト名 + 要約行）、判断（設計との差分）、
    棚卸し I2〜I5 の結果、品質ゲートの結果、コミット一覧。最終応答は「## Subagent Summary」形式（Produced / Key Decisions / Issues / Next Steps）。

## 設計の所在（必要な箇所だけ読む。記録ディレクトリ外のコードは読んでよいが、他 Unit の construction/ 配下は読まない）
- 機能設計: `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/functional-design/{entities,rules,functional-spec}.md`
  （BR2.6 / BR4.1 / BR4.2 / BR5.x、entities の WorkflowDefinitionId / DefinitionRevision / WorkflowDefinition）
- NFR: `.../u2-domain-es-core/nfr-requirements/{security-requirements,tech-stack-decisions}.md`、`.../nfr-design/{security-design,logical-components}.md`
- 契約: `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/contract-design/contract-summary.md`（C4 改訂）、ADR:
  `.../inception/domain-design/decisions.md`（ADR-005 / ADR-008）
- コーディング規則（必読、正本）: `aidlc/spaces/default/knowledge/aidlc-shared/coding-rules/*.md`（field-visibility / module-visibility /
  domain-equality / tell-dont-ask / gateway-taxonomy / use-case-rules / README）
- canon-json の公開 API: `modules/shared/canon-json/src/lib.rs`（`to_value` / `hash_canonical` / `Digest::rendered()`）
- 既存テストの書式: `modules/core/interface-adapter/tests/workflow_definition_repository_impl_test.rs`（tempdir フィクスチャ `tools/data` + `scopes`）
