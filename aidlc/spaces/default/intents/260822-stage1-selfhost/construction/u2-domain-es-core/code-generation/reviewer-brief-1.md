# reviewer-brief-1 — U2 Code Generation（Bolt b51）の独立レビュー（2026-09-07、advisory、iteration 1）

Conversation language: 日本語（コード識別子・固定トークン・YAML キーは英語のまま）

## A. 依頼の条件

- **役割**: `aidlc-architecture-reviewer-agent`（独立レビュー担当）。ステージ `code-generation`、Unit `u2-domain-es-core`
  （ドメインのイベントソーシング中核 — `core-command-domain` の Intent / IntentExecution 集約）。レビュー種別 **advisory**、
  iteration **1**。これは通常フローの **1 回限りの advisory パス**であり、所見は承認ゲートで人間に提示される。修正 → 再レビューの
  ループは後ろに無い。人間が承認前に秤にかけるべき所見だけを、重大度順に報告する。
- **レビュー対象（`review_artifact`）**: `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/code-generation/code-generation-plan.md`。
  ...
  `## Review` より前のバイトは 1 文字も変えない。他のファイルは一切書き換えない・新規作成しない（コードも記録も）。
...
- **必読ファイル**（順に読む。すべて `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/code-generation/` 配下）:
  1. `code-generation-questions.md`（前提 P1〜P5、裁定 Q1 = A / Q2 = A、`## Plan Approval` の承認指紋）
  2. `code-generation-plan.md`（レビュー対象。§1〜§6 + Testing Contract。承認指紋 `sha256:dd1170c1…`）
  3. `unit-test-instructions.md`（Unit 限定コマンド、受入、カバレッジ床）
  4. `code-summary.md`（今回の作業範囲・変更ファイル・判断 11 件・テスト・受入の実測・センサー・申し送り）
  5. `source-manifest.json`（78 パス）と `traceability.json`（49 行）
  6. `developer-report-3.md`（委任 1: FCC 11 型の新設）と `developer-report-4.md`（委任 2: 一斉切替と受入 (a)〜(g)）
  7. 履歴（今回の判定には流用しない）: `code-generation-plan-history-2026-08-23.md`、`code-summary-history-2026-08-23.md`
- **上流（consumes）**: `.../u2-domain-es-core/functional-design/functional-spec.md`（§2 API、§9 引継ぎ #1〜#4）、`.../functional-design/rules.md`
  （BR1.0〜BR5.5）、`.../functional-design/entities.md`（FCC 型の不変条件・操作）、`.../nfr-design/security-design.md`（§2 検査点の二層、§6 決定性と契約、§8 要求対応表）、
  `.../nfr-design/logical-components.md`（§1 追随表、§4 受入手順）、`.../nfr-requirements/security-requirements.md`（NFR1.1〜NFR4.5）、
  `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/contract-design/contract-summary.md`（C3 / C5 / C6）、
  `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/units-generation/unit-of-work.md`（U2 の境界宣言）、
  `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/requirements-analysis/requirements.md`（FR8.3 / FR8.4 / FR1〜FR3、NFR1〜NFR5）。
- **照合する実コード**（コミット `3edf320d` と `dd20266a`、`git diff origin/main..HEAD -- modules` で 78 パス）:
  `modules/core/command/domain/src/orchestration/`（新設 `stage_entries.rs` / `stage_slot.rs` / `stage_slots.rs` / `stage_slots_error.rs` /
  `stage_index_set.rs` / `stage_slug_set.rs` / `artifact_paths.rs` / `transition_steps.rs` / `transition_steps_error.rs` / `review_closures.rs` /
  `pending_iterations.rs`、切替 `intent_execution.rs` / `intent.rs` / `intent_execution_event.rs` / `intent_execution_event/*.rs` /
  `intent_event/created.rs` / `review_attempt.rs` / `report_decision.rs` / `stage_entry.rs` / `apply_error.rs` / `command_error.rs` / `mod.rs`）、
  `modules/core/command/domain/src/workspace/`（`promoted_sections.rs` / `promoted_sections_error.rs` / `rule_lines.rs` / `practices_promotion.rs` /
  `promotion_plan_error.rs` / `mod.rs`）、`modules/core/command/domain/tests/{collection_contract_test,engine_loop_conformance}.rs`、
  `modules/core/infrastructure/src/collections/`（`FirstClassCollection` 契約）、兄弟クレートの追随（`modules/core/command/interface-adapter/src/orchestration/dto/*.rs`、
  `modules/core/read-model-updater/src/{orchestration/dto/*.rs,read_tables.rs,read_tables/*.rs,workspace/projection.rs,workspace/resolved_plan.rs}`、
  `modules/core/command/use-case/src/orchestration/{commit_outcome,commit_verdict_use_case,promote_practices_use_case}.rs`、`modules/app/aidlc/src/{runtime,scaffold}.rs`）、
  `formal/orchestration/engine_loop.qnt`（不変であること）、`Cargo.toml` / `Cargo.lock`（不変であること）、
  `aidlc/spaces/default/knowledge/aidlc-shared/coding-rules/`（README.md、first-class-collections.md、module-visibility.md、field-visibility.md、
  aggregate-commands.md、domain-object-kinds.md、error-handling.md、cqrs-boundaries.md、no-backward-compatibility.md、tell-dont-ask.md、ubiquitous-language.md）。
- **今回の Bolt の趣旨**: functional-spec §9 #1〜#4 の差分 — FCC 11 型の新設、`IntentExecution` の 7 並列列 → `StageSlots`、`Intent` / イベント /
  `PracticesPromotion` / `ReportDecision` / `ReviewAttempt` の FCC 化、`StageEntry::check_plan` の `StageEntries::new` への移設、`next_decision` の
  `Result` 化と `IntentMismatch`、冒頭 doc の是正、兄弟クレート 5 つの追随。DTO のバイト表現は不変、ゴールデン・ITF・Quint は不変。
- **見るべき観点**（advisory。判定閾値はあなたのペルソナに従う）:
  1. **計画との一致**: 承認済み計画 §1 の作るもの 6 点・作らないもの、§2 の 11 型の確定事項（要素・不変条件・操作・`Filtered`・エラー型）、
     §3 の追随対象、§4 の Step 0〜5 が実装と一致するか。`code-summary.md` §3 の逸脱 11 件が計画の意図を損なっていないか（特に
     `recompose(&StageIndexSet)`、`ReviewAttempt::restored(.., Vec<u32>, ..)` の境界例外、`NextAnswerRow::of` の可謬化先、`in_document_order`）。
  2. **コーディング規則との適合**: first-class-collections（要素型を所有する文脈に置く、用途の無い共通メソッドを機械的に足さない、集合型だけが
     combine / divide、境界での `fold_left`）、cqrs-boundaries（リードモデル側が FCC 型をフィールド・戻り値型に持たない）、module-visibility /
     field-visibility、no-backward-compatibility（`stage_keys()` / `check_plan` / `&[..]` 返却 / `#[deprecated]` が 0 件）、error-handling（無言の
     no-op が無い — `OutOfRange` → `ApplyError`、`DuplicateSection`）、domain-object-kinds（4 種以外を作っていない）、`unwrap` / `expect` のプロダクト
     コード不使用。
  3. **契約と決定性**: `# Panics` が `replay` / `apply_event` / 誕生変換 + `workflow_definition.rs` の 4 か所のまま（冒頭 doc の言及を除く）、
     `PlanAction` の所有が `workflow_definition` のみ（BR4.1）、時計利用が `*EventId::generate` のみ（NFR3.1）、`StageIndex::new` が `pub(crate)` のまま、
     `StageSlugSet` の辞書順と投影側の文書順並べ直しがゴールデンで固定されているか、ITF fixture / Quint モデル / DTO の JSON 形が不変か。
  4. **テストと TDD の記録**: `developer-report-4.md` の Red の実出力、RMU 新振る舞い 2 件が「反転で検出力確認」と正直に記録されているか、
     カバレッジ床（クレート 98.87% / ワークスペース 90%）の実測、`PROPTEST_RNG_SEED=20260823` の固定、Monoid 則・差集合則の性質試験。
  5. **記録の整合**: `source-manifest.json` が `git diff --name-only origin/main..HEAD -- modules` と一致するか、`traceability.json` の各 OK target が
     実在し当該要求を実際に担う場所か、`code-summary.md` の数値（78 パス、699 / 2,354 テスト、99.15% / 98.90%）が報告と一致するか。
  6. **センサー実測**: `bun .claude/tools/aidlc-sensor-required-sections.ts --stage code-generation --output-path <plan / unit-test-instructions / code-summary>`、
     `bun .claude/tools/aidlc-sensor-traceability.ts --stage code-generation --output-path <traceability.json>`（`missing_from_upstream_ids` は他 Unit の
     要求 ID で既知のノイズ。`invalid_targets` / `gaps` / `orphans` / `invalid_entries` を見る）。
     読み取り専用の実測として `rg` / `grep` / `sed` / `git diff` / `git log` を使ってよい。`cargo test` / `cargo clippy` / `cargo llvm-cov` の
     実行も可（`PROPTEST_RNG_SEED=20260823` を付ける。時間が掛かる場合は `-p core-command-domain` に限定してよい）。ファイルは書き換えない。
- **書き方**: `## Review` 節は日本語。所見表は `| ID | Severity | Location | Finding | Required action |`（Severity は
  ...
  所見は現行の実装・記録に対して出す。根拠はファイルパスと該当箇所（行番号）で示す。
