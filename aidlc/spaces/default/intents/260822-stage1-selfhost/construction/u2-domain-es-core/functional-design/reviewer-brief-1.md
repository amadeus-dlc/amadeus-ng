# reviewer-brief-1 — U2 Functional Design の独立レビュー（2026-09-07、advisory、iteration 1）

Conversation language: 日本語（コード識別子・固定トークン・YAML キーは英語のまま）

## A. 依頼の条件

- **役割**: `aidlc-architecture-reviewer-agent`（独立レビュー担当）。ステージ `functional-design`、Unit `u2-domain-es-core`
  （ドメインのイベントソーシング中核 — `core-command-domain` の Intent / IntentExecution 集約・ドメインイベント・PlanAction 所有・
  実効計画の畳み込み）。レビュー種別 **advisory**、iteration **1**（この 1 回のみ。再レビューはしない）。
- **レビュー対象（`review_artifact`）**: `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/functional-design/functional-spec.md`。
  この末尾に **`## Review` 節を厳密に 1 つだけ追記**する。節は次を各 1 回だけ含む: `**Verdict:** READY` または
  `**Verdict:** NOT-READY`、`**Reviewer:** aidlc-architecture-reviewer-agent`、`**Iteration:** 1`。
  REQUEST_CHALLENGE の指示が本ブリーフ末尾（§C）にあればその行も 1 回だけ含める。節内の小見出しは H3 以下（`###`）のみ。
  `## Review` より前のバイトは 1 文字も変えない。他のファイルは一切書き換えない・新規作成しない。
- **読む範囲（read scope）**: 下記の必読ファイルと、リポジトリの実コード・形式モデル・コーディング規則のみ。
  **他 Unit の `construction/<other-unit>/` 配下は読まない**（glob / grep / ls でまたがるのも禁止）。intent 記録直下の
  `ls` / glob もしない。
- **必読ファイル**（レビュー対象の成果物と質問票、順に読む）:
  1. `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/functional-design/functional-design-questions.md`
     （Q1〜Q3・P1〜P6 は 2026-08 の記録。**2026-09-07 再走の Q4 / Q4a / Q5・P7〜P10 と `## Consolidated Summary Confirmation` が今回の裁定**）
  2. `.../u2-domain-es-core/functional-design/entities.md`（YAML 正本）
  3. `.../u2-domain-es-core/functional-design/rules.md`（YAML 正本、BR ID）
  4. `.../u2-domain-es-core/functional-design/functional-spec.md`（レビュー対象）
  5. `.../u2-domain-es-core/functional-design/traceability.json`
  6. `.../u2-domain-es-core/correction-report.md`（2026-09-05 の是正記録）
  7. `.../u2-domain-es-core/functional-design/functional-spec-review-history-2026-09-05.md`（是正前のレビュー、履歴。今回の判定に流用しない）
  8. `.../u2-domain-es-core/functional-design/pending-revision.md`（2026-08-23 の改訂案、履歴。現行 API の導入指示ではない）
- **上流（consumes、共有正本）**: `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/units-generation/unit-of-work.md`（U2 節）、
  `inception/units-generation/unit-of-work-story-map.md`、`inception/requirements-analysis/requirements.md`、
  `inception/domain-design/components.md`、`inception/domain-design/decisions.md`、`inception/contract-design/contract-summary.md`（C3 / C5）。
- **照合する実コード・モデル・規則**（設計は「現行コード + 引継ぎ差分」を記述する。設計と現行コードの差は functional-spec §9 に列挙されているはず）:
  `modules/core/command/domain/src/orchestration/`（`intent.rs` / `intent_execution.rs` / `intent_execution_event.rs` と同名ディレクトリ /
  `stage_entry.rs` / `stage_key.rs` / `review_attempt.rs` / `report_decision.rs` / `mod.rs`）、
  `modules/core/command/domain/src/workflow_definition/workflow_definition.rs`（残す述語）、
  `modules/core/command/domain/src/workflow_definition/stage_graph.rs` / `scope_grid.rs`（FirstClassCollection 実装）、
  `modules/core/command/domain/src/workspace/practices_promotion.rs`、
  `modules/core/infrastructure/src/collections/`（`first_class_collection.rs` / `collection.rs` / `non_empty_collection.rs`）、
  `modules/core/read-model-updater/src/read_tables/next_answer_row.rs` / `modules/core/read-model-updater/src/workspace/resolved_plan.rs`
  （`next_decision` と `stages()` の現行呼出側）、`formal/orchestration/engine_loop.qnt`（v2.7 ヘッダ）、
  `aidlc/spaces/default/knowledge/aidlc-shared/coding-rules/`（README.md、first-class-collections.md、aggregate-commands.md、
  aggregate-references.md、domain-persistence-neutrality.md、cqrs-boundaries.md、tell-dont-ask.md、domain-object-kinds.md）。
- **今回の再走の趣旨**: 2026-09-05 に是正済みの設計本文を出発点に、以下を反映した。これらが成果物 4 本に**矛盾なく・過不足なく**
  反映されているかを見る。
  - Q4 = X（オーナー回答）「リードモデルでは使わない。コマンド側でドメインモデルの配列部分があるなら FCC を使う」→ BR5.5 と
    entities.md の新 FCC 型（StageEntries / StageSlots / StageIndexSet / ArtifactPaths / StageSlugSet / PromotedSections / RuleLines）。
  - Q4a = A（オーナー追問「結合・差集合の高階操作が無いとイテレータで取り出してロジックが分散する」）→ 全 FCC に
    `combine` / `divide` を型ごとの契約として定義。共通 trait への一律化は積み残し（オーナーの最終方針）。
  - Q5 = A → `next_decision(&Intent, &NextRequest) -> Result<NextDecision, CommandError>`、不一致は `IntentMismatch`。
  - P7〜P10 → 上流の古い再生方式記述（2026-08-30「ジャーナル全再生」）は 2026-09-05 裁定（最新スナップショット + 差分）で
    上書き済みとして扱う、`orchestration/mod.rs` 冒頭の旧説明の修正を引継ぎ、旧レビュー節の履歴退避、2026-09-06 のコレクション型の事実記録。
- **見るべき観点**（advisory。判定閾値はあなたのペルソナに従う）:
  1. 4 成果物の内部整合: entities.md の YAML（型・操作名）と rules.md の BR（特に BR1.2 / BR2.2 / BR2.6 / BR3.1 / BR4.2 / BR5.4 / BR5.5）、
     functional-spec.md の API 表・W1〜W7・§4〜§6・§9 が同じ型名・操作名・戻り値を使っているか。
  2. 裁定の忠実な反映: Q4 / Q4a / Q5 の回答と `## Consolidated Summary Confirmation` の内容が、成果物で薄められたり拡大解釈されたりしていないか
     （例: リードモデル側に FCC を要求していないか、combine / divide の意味論が型種別ごとに規則どおりか）。
  3. 現行コードとの差分の網羅: §9 の引継ぎ表が、設計と現行コードの差を漏れなく挙げているか（`Vec` / `&[..]` の公開箇所、
     `next_decision` の署名、呼出側の追随、doc の乖離）。設計が「現行コードにある」と書いたものは実在するか。
  4. コーディング規則との適合: first-class-collections.md（操作の意味論、Monoid 則、機械的なメソッド追加の禁止）、
     aggregate-references.md（ID 照合）、aggregate-commands.md（1 コマンド 1 イベント、再生の形）、domain-object-kinds.md（4 種）。
  5. 上流との整合: unit-of-work U2 節・requirements FR8.3 / FR8.4 / FR1.3 / FR2.1 / FR3.1 / FR3.3 / NFR1 / NFR3 と traceability.json の対応。
     上流に残る古い記述（P7）を設計が黙って読み替えていないか（明示して扱っているか）。
  6. Quint v2.7 射影表（rules.md 第 3 節）が FCC 化で意味論を変えないという主張の妥当性。
  7. センサー実測: `bun .claude/tools/aidlc-sensor-required-sections.ts --stage functional-design --output-path <各 md>`（3 本）、
     `bun .claude/tools/aidlc-sensor-traceability.ts --stage functional-design --output-path <traceability.json>`
     （`missing_from_upstream_ids` は他 Unit の ID による既知ノイズ。`gaps` / `orphans` / `invalid_targets` / `invalid_entries` を見る）。
     必要なら `cargo test --locked -p core-command-domain --lib orchestration` 等の現行実測（読み取り専用の確認として）。
- **書き方**: `## Review` 節は日本語。所見表は `| ID | Severity | Location | Finding | Required action |`（Severity は
  ...
  所見は成果物の現行本文に対して出す（是正前の旧レビューの所見番号を引き継ぐ必要はない）。根拠はファイルパスと該当箇所で示す。
