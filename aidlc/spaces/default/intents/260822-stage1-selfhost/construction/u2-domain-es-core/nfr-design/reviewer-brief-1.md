# reviewer-brief-1 — U2 NFR Design の独立レビュー（2026-09-07、advisory、iteration 1）

Conversation language: 日本語（コード識別子・固定トークン・YAML キーは英語のまま）

## A. 依頼の条件

- **役割**: `aidlc-architecture-reviewer-agent`（独立レビュー担当）。ステージ `nfr-design`、Unit `u2-domain-es-core`
  （ドメインのイベントソーシング中核 — `core-command-domain` の Intent / IntentExecution 集約）。レビュー種別 **advisory**、
  iteration **1**（この 1 回のみ。再レビューはしない）。
- **レビュー対象（`review_artifact`）**: `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/nfr-design/security-design.md`。
...
- **必読ファイル**（順に読む）:
  1. `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u2-domain-es-core/nfr-design/nfr-design-questions.md`
     （2026-08-23 の前提 P1〜P4・まとめは履歴。**2026-09-07 再走の P5〜P11 と `## Consolidated Summary Confirmation` が今回の確認事項**）
  2. `.../u2-domain-es-core/nfr-design/security-design.md`（レビュー対象。§1〜§8）
  3. `.../u2-domain-es-core/nfr-design/logical-components.md`（§1 コンポーネント一覧 + 兄弟クレートへの追随表、§2〜§5）
  4. `.../u2-domain-es-core/nfr-design/traceability.json`（NFR1.1〜NFR4.5 + NFR2.5 の 17 ID）
  5. `.../u2-domain-es-core/nfr-design/security-design-review-history-2026-08-23.md`（旧世代のレビューと改訂案、履歴。今回の判定に流用しない）
  6. `.../u2-domain-es-core/nfr-design/validation-20260907.md`（本再走の実測記録: センサー結果、実測件数、質問票 P7 の概数誤り）
- **上流（consumes）**: `.../u2-domain-es-core/nfr-requirements/security-requirements.md`（NFR1.1〜NFR4.5、末尾の 2026-09-06 レビュー節
  R-01〜R-08 — advisory NOT-READY、成果物は凍結中）、`.../u2-domain-es-core/nfr-requirements/tech-stack-decisions.md`（§1 FCC 行、§2 依存の差分、
  §3 未決）、`.../u2-domain-es-core/functional-design/functional-spec.md`（§2 API、§9 引継ぎ、末尾の 2026-09-06 レビュー節 R-01〜R-10 —
  advisory NOT-READY、凍結中）、`.../u2-domain-es-core/functional-design/rules.md`（BR1.0〜BR5.5）、`.../u2-domain-es-core/functional-design/entities.md`
  （FCC 型の不変条件・操作）、`aidlc/spaces/default/intents/260822-stage1-selfhost/inception/contract-design/contract-summary.md`（C3 / C5 / C6）、
  `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/units-generation/unit-of-work.md`（U2 の境界宣言）。
- **照合する実コード・設定**: `modules/core/command/domain/Cargo.toml`、`modules/core/command/domain/src/orchestration/`（`mod.rs` の
  `mod` / `pub use` 列挙と冒頭 doc、`intent.rs`、`intent_execution.rs`、`intent_execution_error.rs`、`command_error.rs`、`report_refusal.rs`、
  `review_attempt.rs`、`report_decision.rs`、`stage_entry.rs`、`intent_event/created.rs`、`intent_execution_event/*.rs`）、
  `modules/core/command/domain/src/workspace/`（`mod.rs`、`practices_promotion.rs`、`promoted_section.rs`）、`modules/core/command/domain/tests/`
  （`engine_loop_conformance.rs` / `collection_contract_test.rs`）、`modules/core/infrastructure/src/collections/`、
  兄弟クレートの呼出箇所（読み取りのみ）: `modules/core/command/interface-adapter/src/orchestration/dto/{intent_dto,created_dto,intent_execution_dto,intent_execution_event_dto}.rs`、
  `modules/core/read-model-updater/src/{read_tables.rs,read_tables/stage_lookup.rs,read_tables/next_answer_row.rs,workspace/resolved_plan.rs}`、
  `modules/core/command/use-case/src/orchestration/{commit_verdict_use_case.rs,test_support.rs}`、
  `formal/orchestration/engine_loop.qnt`（ヘッダの版）、`scripts/coverage.sh`、`Cargo.toml`（`[workspace.lints]`）、
  `aidlc/spaces/default/knowledge/aidlc-shared/coding-rules/`（README.md、module-visibility.md、first-class-collections.md、
  aggregate-commands.md、aggregate-references.md、domain-persistence-neutrality.md、error-handling.md、cqrs-boundaries.md）。
- **今回の再走の趣旨**: 2026-08-23 の旧世代 NFR 設計（`WorkflowExecution`・`from_snapshot` / `SnapshotError` / 公開 `ApplyError`・
  「どの検査も panic なし」・`next_decision` の `DefinitionMismatch`・`modules/core/domain/` 配置・B3 の `find_by_id` 範囲拡張）を、
  後続裁定（Intent / IntentExecution 分離、16 変種、最新スナップショット + 差分再生、壊れた歴史は panic、`IntentMismatch`、BR5.5 FCC 化、
  Q4 / Q4a / Q5）と現行コード `modules/core/command/domain/` へ同期した。検査点を二層（Err / panic）に整理し、新設 FCC の置き場
  （orchestration / workspace）、兄弟クレートへの追随一覧と越境の裁定根拠、テスト配置と受入手順（カバレッジの二重計測、BR4.1 判定式の
  `-e` 2 本）を定めた。上流 2 段（functional-design / nfr-requirements）は advisory NOT-READY で凍結中であり、本設計はその所見
  （FD R-01〜R-10、NFR R-01〜R-08）を引き継ぐ側の記述を含む。
- **見るべき観点**（advisory。判定閾値はあなたのペルソナに従う）:
  1. **実装可能性**: 開発者が設計者へ追加照会せずに実装へ入れるか。特に新設 FCC の置き場・公開面・依存方向、兄弟クレートへの追随の
     網羅（実測の呼出箇所と一致するか）、`ReviewAttempt` 内部列と `TransitionSteps` の未決（FD R-01）の扱いが明示されているか。
  2. **実測との一致**: `orchestration/` のエントリ数、`IntentExecution` の 7 並列列、生の `Vec` / `&[..]` 公開箇所、`CommandError` /
     `ReportRefusal` の変種数、`IntentExecutionError` の形（struct）、`ApplyError` の可視性、`# Panics` の所在（`replay` / `apply_event`）と
     `intent_execution.rs` 冒頭 doc の乖離、`workspace` → `orchestration` 参照 0 件、`matches` の位置、時計利用箇所。
  3. **上流との整合**: security-requirements.md の NFR1.1〜NFR4.5 と NFR2.5 が traceability.json で過不足なく被覆され、各 target が
     成果物の節を指すか。凍結中の NFR 要求レビュー所見（R-01 RMU の FCC 参照、R-02 use-case の追随、R-03 呼出元と越境裁定、R-04 判定式、
     R-05 型集合、R-06 combine の用途、R-07 カバレッジ希釈、R-08 検収先）と機能設計レビュー所見（R-01〜R-10）を本設計がどう受け止めて
     いるか — 黙って読み替えていないか。
  4. **コーディング規則との適合**: module-visibility（private mod + ファサード `pub use`）、first-class-collections（要素型を所有する文脈、
     用途の無い combine / divide を足さない、境界での `fold_left`）、aggregate-commands（再生の形、panic の射程）、error-handling、
     cqrs-boundaries（リードモデル側は FCC 型を定義・保持しない）。
  5. **設計ステージの制約**: コード例は ≤15 行の例示に留まっているか。
  6. **センサー実測**: `bun .claude/tools/aidlc-sensor-required-sections.ts --stage nfr-design --output-path <各 md>`（2 本）、
     `bun .claude/tools/aidlc-sensor-traceability.ts --stage nfr-design --output-path <traceability.json>`。
     必要なら読み取り専用の確認として `rg` / `grep` / `sed` によるコード実測。`cargo` の実行は不要（ビルド・テストは走らせない）。
