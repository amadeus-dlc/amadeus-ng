# reviewer-brief-1 — U10 Code Generation の独立レビュー（2026-09-06、advisory、iteration 1）
...
### A.2 対象（Unit: `u10-ci-governance`、stage: `code-generation`）

記録ディレクトリ: `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u10-ci-governance/code-generation/`

- ステージ定義: `.claude/aidlc-common/stages/construction/code-generation.md`
- Q&A: `<記録ディレクトリ>/code-generation-questions.md`
- 生成物（`produces`）:
  - `<記録ディレクトリ>/code-generation-plan.md` — **レビュー節を追記する対象**（`directive.review_artifact`）
  - `<記録ディレクトリ>/unit-test-instructions.md`
  - `<記録ディレクトリ>/code-summary.md`
  - `<記録ディレクトリ>/traceability.json`
- 付随: `<記録ディレクトリ>/source-manifest.json`（`writes` は空 — 今回ワークスペースのアプリケーション側ファイルは作成・変更・削除していない）
- 上流（`consumes`、パスのみ）:
  - `aidlc/spaces/default/intents/260822-stage1-selfhost/inception/contract-design/contract-summary.md`
  - `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u10-ci-governance/nfr-design/security-design.md`
...
- 同 Unit 内の参考（読んでよい）: `../nfr-requirements/security-requirements.md`・`tech-stack-decisions.md`（2026-09-06 改訂、READY）、
  `../revision-baseline-20260906.md`、`../ruleset-observed-20260906.json`、記録ディレクトリ内の履歴ファイル
  （`code-summary-history-2026-08-23.md`、`code-generation-plan-history-2026-08-22.md`、`unit-test-instructions-history-2026-08-22.md`、
  `superseding-decisions.md`、`pending-revision.md`、`ruleset/`）。
- **読まないもの**: `construction/code-generation/memory.md`（日誌）、`developer-brief-*.md`、`developer-report-*.md`、他 Unit の
  `construction/<other-unit>/` 配下（glob・grep でまたぐことも不可）。

### A.3 今回の作業の性質（レビューの前提）

- CI・品質管理の実装は Bolt B2（PR #25・#26）で `main` へ反映済み。今回はワークスペースのファイルを**変更していない**（`git status` で
  ワークスペース側の差分ゼロ）。行ったのは (1) 現行設定を 2026-09-06 改訂の要件・設計へ照合、(2) Unit 限定コマンドと受入の実測
  （カバレッジ 2 回測定・`cargo audit` 2 件・unsafe 不適合例の拒否 4 件）、(3) 凍結されていた `code-summary.md`・`traceability.json` の
  現行の事実への書き直しと `source-manifest.json` の作成。
- 2026-08-23 の回復レビュー（NOT-READY: Critical 1・Major 2 — review-thread ゲート未記載、traceability の target に注記混在、
  4 コンテキスト化未反映）は履歴ファイル `code-summary-history-2026-08-23.md` 末尾にある。今回の成果物がそれらを解消しているかは
  検証対象だが、履歴の所見 ID を引き継ぐ必要はない（新しい run のため）。
- 上流要件 `security-requirements.md` の R-01（Markdown 表 2 行の表示崩れ、Minor）は上流の未解決所見として残る。本ステージの成果物が
  同種の崩れを持ち込んでいないかは確認してよいが、上流所見を本ステージの判定で解消扱いにしない。
...
- `linter` / `type-check` センサーは TS/JS の生成コードが対象。今回は Markdown/JSON のみのため対象外と記録してよい。
- 実測の再確認（推奨、いずれも読取のみ・副作用なし）: `bash scripts/governance/verify-ci-governance.sh --with-ruleset`（`gh` の GET のみ）、
  `bash scripts/governance/toolchain-inputs.sh`、`cargo test --manifest-path tools/lint/Cargo.toml`。カバレッジの再測定
  （`scripts/coverage.sh`、1 回 1 分前後）は任意。`ruleset-required-checks.sh` は実行しない。GitHub への書込は行わない。
- 実測ログ（開発担当が保存、読取可）: `/private/tmp/claude-501/-Users-j5ik2o-orca-workspaces-amadeus-ng-stage1-selfhost/bdae4b2f-d1d9-470f-bf7c-df8853392e07/scratchpad/step*.log`

### A.5 確認してほしい観点（例示であり、限定ではない）

1. `code-summary.md` の記述が実ファイル（`.github/workflows/ci.yml`、`.github/workflows/review-thread-resolution.yml`、`scripts/coverage.sh`、
   `rust-toolchain.toml`、`Cargo.toml`、`tools/lint/Cargo.toml`、`scripts/governance/*.sh`）と GitHub ruleset の観測記録に一致するか。
   過去の事実（暫定 0.05、残差 0.0175pp、31 本、検査 15 項目、PR #25/#26 の出来事）を今回の実施として書いていないか。
2. `traceability.json` の 15 件（FR9.1〜9.5、NFR2.1〜2.5、NFR4.1〜4.5）が要件・Unit 割当と一致し、各 `OK` の target が実在ファイル
   単体で、その要求の実装先として妥当か。
3. 実測の記載（カバレッジ 99.14022164135578% × 2、差 0.00、`cargo audit` 125/5 crate、unsafe 拒否 4 件、自己テスト 93 本、検査 20 項目）が
   ログと一致し、未検証範囲（全 CI 実行、キューの成功/失敗両経路、レビュー再評価の反映、外部ワークフロー内部、相対ゲート、
   ruleset スクリプトの書込経路）が過大主張なく区別されているか。
4. 計画 `code-generation-plan.md` の Step 1〜6 と Testing Contract の適用（packaging Unit、新規プロダクションコードなし）が
   ステージ定義と矛盾しないか。`unit-test-instructions.md` のコマンドが Unit 限定か。
5. `source-manifest.json` の `writes` が空であることが事実と一致するか（ワークスペース側の差分ゼロ）。
...
- 節の中に、レンダリングされる行として次を**それぞれ 1 回だけ**含める:
  ...
  依頼書に `Request Challenge` の値が与えられている場合のみ `**Request Challenge:** <値>` を 1 回書く（与えられていなければ書かない）。
...
- 他の成果物（`code-summary.md` など）には**書き込まない**。
