...
# 開発ブリーフ（委任 1 / 2）— U1 canon-json とゴールデン、計画 Step 1〜16
...
- あなたは aidlc-developer-agent。承認済み計画（下記 §6、Step 1〜16 だけ）を**順に**実行し、実装・テスト・設定を
  ワークスペースルートに書く。記録ディレクトリ（`aidlc/spaces/default/intents/260822-stage1-selfhost/`）には
  コードを置かない。Step 17〜19（CLI / フックゴールデン）は別委任なので触らない。
- 計画ファイル `aidlc/spaces/default/intents/260822-stage1-selfhost/construction/u1-canon-json-goldens/code-generation/code-generation-plan.md`
  の該当チェックボックスを、各 Step 完了時に `[x]` にする（Step 0 と Step 17〜19 は触らない）。
- 下記 §5 の **Testing Contract は Part 2 の権威**。メモリ規則を独自に再解釈しない。TDD: 各 Red は失敗コマンド出力を
  記録してから Green に進む。
- 作業ブランチは `bolt/b1-u1-canon-json-goldens`（作成済み）。意味単位でコミットする（`feat(canon-json): …`、
  `test(goldens): …`、`chore(workspace): …`）。**push / PR / ブランチ切替はしない。** ワークフローの承認ゲートや
  `aidlc-*.ts` の状態変更コマンドは呼ばない（`aidlc/` 配下のメモリ・状態ファイルも編集しない）。
- 質問が必要なときは止まって返答に書く（推測で設計判断を変えない）。採取に失敗したゴールデンは欠落として理由付きで
  記録する — 捏造しない。
- 終了時の返答（あなたの最終テキスト = 戻り値）は §8 の形で書く。

## 1. ワークスペース

- Rust edition 2024、Cargo workspace resolver 3（`Cargo.toml`）。クリーンアーキテクチャ（層 = クレート）。
  `canon-json` は `modules/shared/canon-json`（現在は `lib.rs` に `#![forbid(unsafe_code)]` と doc のみのスタブ）。
- lint: `cargo fmt --all --check` → `cargo clippy --workspace --all-targets -- -D warnings`（workspace lints 47 ルール deny:
  `unwrap_used` / `expect_used` / `missing_docs` / `unreachable_pub` / `todo` / `unimplemented` / `print_stdout` /
  `needless_pass_by_value` / `must_use_candidate` / `missing_const_for_fn` 等）→ `cargo lint`（`tools/lint`、
  coding-rules の機械強制）→ `cargo test --workspace`。`clippy.toml` はテストのみ `unwrap`/`expect` 許可。
- 既存依存: serde 1（derive）・serde_json 1（core-interface-adapter、core-domain の dev-dep）、md5、nix、libc、
  proptest、tempfile。`rust-toolchain.toml` は無い（U10 で追加予定）。
- ツール: cargo 1.95.0 / rustc 1.95.0、bun 1.3.13、node 24、ネットワーク可（raw.githubusercontent.com へ HTTP 200 実測）。
- 既存ゴールデン: `tests/golden/upstream-3c3146cf/{README.md,stage-graph.json,scope-grid.json}` — **既存 3 ファイルの
  バイトは変更禁止**（README には節の**追記**のみ可）。
- upstream ピン: awslabs/aidlc-workflows `3c3146cfd7cef33020d48e8d48d4e80d0f8c2820`（v2.6.40）。`canonicalize` /
  `sha256` / `hashObject` は `dist/claude/.claude/tools/aidlc-testing-posture.ts` の 104〜123 行付近
  （upstream 仕様 `docs/upstream/specs/09-cli-tools.md` §8.4）。参考: 本リポジトリにインストール済みのシェルは
  2.6.54（`.claude/tools/aidlc-testing-posture.ts:104-123` に同名関数）だが、**採取はピン 3c3146cf のコードで行う**。

## 2. 読むべき設計成果物（本 Unit のみ）

すべて `aidlc/spaces/default/intents/260822-stage1-selfhost/` 配下:

- `construction/u1-canon-json-goldens/functional-design/functional-spec.md` — W1〜W5、インターフェイス、状態遷移
- `construction/u1-canon-json-goldens/functional-design/rules.md` — BR1.1〜BR1.8 / BR2.1〜BR2.5（正本 yaml）
- `construction/u1-canon-json-goldens/functional-design/entities.md` — JsonValue / SerializationProfile / Digest / GoldenCase / NormalizationRule / GoldenCorpus
- `construction/u1-canon-json-goldens/nfr-requirements/security-requirements.md` — NFR1.x / NFR2.x / NFR4.x
- `construction/u1-canon-json-goldens/nfr-requirements/tech-stack-decisions.md` — クレート配置・依存・数値表現・機械強制
- `construction/u1-canon-json-goldens/nfr-design/security-design.md` — parse 境界・ParseError・深さ上限・サプライチェーン
- `construction/u1-canon-json-goldens/nfr-design/logical-components.md` — 6 モジュール + ファサード + 比較器の置き場
- `construction/u1-canon-json-goldens/code-generation/code-generation-questions.md` — Q1 = A / Q2 = A / Approve Plan

Inception 成果物（必要なら Read）:
- `inception/requirements-analysis/requirements.md` — FR7.1〜7.3（受入表採取 / CLI ゴールデン / canon-json 実装）、NFR1 upstream 互換、NFR2 品質ゲート、NFR4 サプライチェーン
- `inception/units-generation/unit-of-work.md` — U1 の責務・境界・合格（受入表全行一致 + FR7.2 コミット）
- `inception/contract-design/contract-summary.md` — C7 ゴールデン契約（layout は `tests/golden/upstream-3c3146cf/…`、Q2 = A で改訂済み）、C1 の continue_token（消費側）
- `inception/domain-design/components.md` — CanonJson は依存ゼロの純粋部品（外部クレート依存は別軸）
- `docs/adr/0001-canonical-json-serializer.md` — 決定 1〜6、受入条件 (a)〜(e)

## 3. コーディング規則（正本、必読）

`aidlc/spaces/default/knowledge/aidlc-shared/coding-rules/` の README と全ルールファイルを**コードを書く前に読む**。
要点: フィールド既定 private・アクセサ公開 / モジュール既定 private・`pub use` ファサード・**利便再エクスポート禁止** /
ドメイン同値は `PartialEq`/`Eq`（名前付き比較メソッド禁止）/ Repository 造語禁止 / Tell-Don't-Ask / use-case 規則。
エラーは手実装 enum + `fmt::Display`（thiserror / anyhow 不使用）。
