...
entities:
  - name: JsonValue
    ...
    attributes:
      ...
      - { name: integer_value, type: integer(i64/u64), required: false }
      - { name: float_value, type: float(f64), required: false, constraints: "非有限（NaN / ±Infinity）は直列化時に null" }
      - { name: string_value, type: string(UTF-8), required: false }
  ...
  - name: SerializationProfile
    ...
    attributes:
      ...
      - { name: key_order, type: enum, required: true, allowed_values: [declared-or-insertion, recursive-sorted], constraints: "hash-canonical のみ recursive-sorted" }
