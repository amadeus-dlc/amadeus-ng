#!/usr/bin/env bun
/**
 * 裁定 Q1 = B（`ReviewPolicy::per_unit` の配線）の**配線後**の突き合わせ。
 *
 * `upstream-capture-logs/capture-q1-review-freeze.ts`（`u2_upstream_capture` 採取）の派生で
 * ある。手順・入力・正規化はそのままで、条件 B に**Unit 宛先の探査を 2 つ足した**だけである。
 * 足した理由は、配線後に残る差の所在を推測ではなく実測で言うためである:
 *   - `unit-path-under-stage-receipt` — ステージ水準の受領証しか無いときの Unit 宛先。
 *   - `sibling-unit-under-stage-receipt` — 同じ状態での別 Unit 宛先。
 *
 * 元の説明（第 2 版）:
 *
...
const CASES: Case[] = [
  ...
  },
  {
    ...
    probes: [
      ...
      { id: "unit-path-under-stage-receipt", relative: "construction/u1-x/code-generation/code-generation-plan.md", expectation: "ステージ水準の受領証しか無いときの Unit 宛先" },
      { id: "sibling-unit-under-stage-receipt", relative: "construction/u2-y/code-generation/code-generation-plan.md", expectation: "同じ状態の別 Unit 宛先" },
    ],
  },
  {
...
writeFileSync(destination, `${JSON.stringify({
  question: "Q1=B の配線後の突き合わせ（review-freeze の凍結範囲）",
