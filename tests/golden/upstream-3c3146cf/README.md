...
> **注記（2026-09-07）**: 本 README 中の `docs/...` は 2026-09-07 に削除した手書き文書を指す履歴的参照である。
> 現行の正本は本ディレクトリのバイト列とパリティテストのアサート値である。
...
このディレクトリごと新しいコミット ID のディレクトリを作り、ゴールデンの差分を全数審査する
（観測可能な差は逸脱として記録する。旧 `docs/specs/00-policy.md` のピン留め方針は 2026-09-07 に削除した）。フォーマッタ・リンタ・エディタの自動整形が
...
旧 `docs/specs/research/golden-3c3146cf-graph-dist.md`（2026-09-07 に削除）に記録してあった。
パリティテストのアサート値はすべてそのレポートの実測に由来し、以後はパリティテストのアサート値が正本である。
...
先に当て、その後に時刻とセッションの規則を適用する。CLONEの形による推測は廃止し、
実測して渡したシャード名・ホスト名だけを置換する。
