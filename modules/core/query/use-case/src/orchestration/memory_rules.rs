...
//! `org → team → project → phases/<phase>` (strict-additive) — `base` の並びは読んだ側が
//! 解決順で渡し、本型はその順序を保持してフェーズルールを後置する。
...
pub struct MemoryRules {
    ...
    phases: BTreeMap<PhaseView, RuleContent>,
}
...
impl MemoryRules {
    ...
    /// directive を出す (02 §10)。分割不能はパース時ではなくパック時に判明するので、
    /// 失敗の表現は読取結果の分類ではなく本メソッドの戻り値にある。
    pub fn plan_for(&self, phase: PhaseView) -> Result<SteeringPlan, UnsplittableSection> {
...
mod tests {
    ...
    fn the_plan_reads_base_then_the_phase_rule_in_resolution_order() {
        ...
        let plan = rules.plan_for(PhaseView::Inception).unwrap();
    ...
    fn a_phase_without_a_rule_file_plans_the_base_only() {
        ...
        let plan = rules.plan_for(PhaseView::Initialization).unwrap();
    ...
    fn a_phase_rule_of_another_phase_is_not_delivered() {
        let rules = MemoryRules::new(
            ...
            [(
                ...
                content("memory/phases/construction.md", "# Construction\n"),
            )]
            .into_iter()
            .collect(),
        );
