...
//! これを読む・パースする責務は**クエリサイド**である (オーナー裁定 2026-08-30 —
//! コマンド側に置くのは CQRS 違反)。ファイルの読取・パス解決・env オーバライドの解決・
//! scopes ディレクトリの列挙は同クレートの reader ([`super::workflow_definition_reader`])
//! が行い、本モジュールは **fs 呼び出しゼロ**の変換だけを持つ。
//!
//! **このモジュールが所有するもの** (12 §6):
