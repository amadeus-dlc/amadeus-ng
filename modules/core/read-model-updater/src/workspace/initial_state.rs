//! Createdから投影する初期状態。本家2.7.1の初期化表現。
...
/// `Stages to Skip` の greenfield 調整の注釈（upstream `intent-create` の
/// `` `${reStage.number} (reverse-engineering — greenfield)` `` — `aidlc-utility.ts:5792`
/// @a277af21。区切りは U+2014）。畳んだ事実は intent の計画 (`StageEntry::is_greenfield_adjusted`)
/// が運ぶので、ここは綴りだけを持つ。
const GREENFIELD_SKIP_ANNOTATION: &str = " — greenfield";
...
#[must_use]
pub(crate) fn compose(intent: &Intent, project_root: &str, started_at: &str) -> String {
    ...
    push_field(&mut out, "Project", &project_preview(intent.request()));
    push_field(
        &mut out,
...
/// 本家2.7.1のProject欄は単一行のプレビューで、原文は別のJSON投影に保つ。
fn project_preview(request: &str) -> String {
    ...
    let unsafe_char =
        |c: char| c <= '\u{1f}' || c == '\u{7f}' || c == '\u{2028}' || c == '\u{2029}';
    if !request.chars().any(unsafe_char) {
    ...
    {
        if c != ' ' || !preview.ends_with(' ') {
            preview.push(c);
        }
    }
    let preview = preview.trim_matches(|c: char| c.is_whitespace() || c == '\u{feff}');
    if preview.is_empty() {
...
fn push_stage_progress(out: &mut String, intent: &Intent) {
    for phase in PHASES {
        let stages = intent.stages().filter(|entry| entry.phase() == phase);
        out.push_str(&format!(
            "\n### {} PHASE\n",
            phase_label(phase).to_uppercase()
        ));
        if phase == PhaseId::Construction {
            out.push_str("Per unit: [TBD]\n");
        }
        stages.fold_left((), |(), stage| {
            let action = if stage.plan_action() == PlanAction::Execute {
                "EXECUTE"
            } else {
                "SKIP"
            };
...
fn slugs_of(intent: &Intent, plan_action: PlanAction) -> Vec<String> {
    intent
        ...
        .fold_left(Vec::new(), |mut slugs, entry| {
            slugs.push(if plan_action == PlanAction::Execute {
                entry.display().number().as_str().to_string()
            } else {
                format!(
                    "{} ({}{})",
                    entry.display().number().as_str(),
                    entry.slug().as_str(),
                    if entry.is_greenfield_adjusted() {
                        GREENFIELD_SKIP_ANNOTATION
                    } else {
                        ""
                    }
                )
...
fn first_post_initialization(intent: &Intent) -> Option<StageEntry> {
    intent.first_post_initialization()
}
...
mod tests {
    ...
    #[test]
    fn skipped_stages_keep_their_number_and_pending_skip_row() {
        ...
        assert!(state.contains("- **Stages to Skip**: 1.1 (market-research)"));
        assert!(state.contains("- [ ] market-research — SKIP"));
        assert!(state.contains("- **Stages to Execute**: 0.1, 2.1"));
        assert!(state.contains("- **Total Stages**: 2"));
    }

    /// greenfield 調整で畳んだステージだけが ` — greenfield` の注釈を持つ (upstream
    /// `2.1 (reverse-engineering — greenfield)`)。素のグリッド SKIP には付かない。
    #[test]
    fn a_greenfield_adjusted_stage_is_annotated_in_the_skip_list() {
        let intent = intent(vec![
            stage(
                "state-init",
                "0.1",
        ...
        assert!(
            state.contains(
                "- **Stages to Skip**: 1.2 (market-research), 2.1 (reverse-engineering — greenfield)"
            ),
            "{state}"
        );
        assert!(
            state.contains("- [ ] reverse-engineering — SKIP"),
            "{state}"
        );
        assert!(
            state.contains("- **Stages to Execute**: 0.1, 2.6"),
            "{state}"
        );
        assert!(state.contains("- **Total Stages**: 2"), "{state}");
        assert!(
            state.contains("- **Active Agent**: orchestrator"),
            "{state}"
        );
    }
