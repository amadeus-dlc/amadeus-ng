//! `read_jump_result` の行を組む投影 — 移動イベントとその前後から [`JumpResultRow`] を組む。
...
use crate::orchestration::{JournalBatch, JumpResultRow};

/// 実行の履歴を順に再生し、移動イベントごとに移動前後の状態から結果を 1 行へ写す。
///
/// # Errors
///
/// 実行の誕生記録より前に別のイベントが現れた場合、または移動先が移動前の実行で
/// 解決できない場合 ([`ReadTablesError::MissingGenesis`])。
pub(super) fn rows(history: &JournalBatch) -> Result<Vec<JumpResultRow>, ReadTablesError> {
