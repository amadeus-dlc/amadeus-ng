...
//! 持っているのは集約の添字帳 ([`IntentExecution::stage_keys`]) なので、引き方をここに
//! 1 つだけ置いて、行ごとに書き直さない。
//!
//! 生の添字アクセスをせず `get` で引くのは、`clippy::indexing_slicing` を避けるためだけ
//! ではない — 範囲外の位置は「答えが無い」であって panic ではないからである。
//!
//! [`IntentExecution::stage_keys`]: core_command_domain::orchestration::IntentExecution::stage_keys
