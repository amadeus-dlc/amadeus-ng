...
//! RMU は 2 つの層でできている。
//!
//! 1. **取得ループ** [`orchestration::OrchestrationReadModelUpdater`] ほか面ごとの更新器 —
//!    処理したシーケンス番号を読み、`events_after` で差分を引き、投影核へ渡し、表の DAO で
//!    書いて番号を進める。ストレージ・接続・チェックポイントを知るのはこちらだけである。
...
//! # 集約ごとに独立した投影単位がある
//!
//! [`read_tables`] の 20 表とは別に、**自分の manifest だけを投影する**小さな投影単位が
