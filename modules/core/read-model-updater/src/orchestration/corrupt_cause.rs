//! `CorruptCause` — 「行は読めたがドメインへ写せない」理由の分類、**クエリ側の閉集合**。
...
/// ある。両側は互いを知らないので (`coding-rules/cqrs-boundaries.md`)、分類を 1 つの enum で
/// 共有すると、どちらか一方がもう一方を `Cargo.toml` に書くことになる。DRY より側の独立を
/// 採り、**実際に起きうる変種だけ**を各側が持つ (無用な変種は「この面ではありえない」という
/// 情報を消してしまう)。
///
/// クエリ側に無いのはコマンド側の再水和にしか意味の無い 2 つ — スナップショット行の欠落
