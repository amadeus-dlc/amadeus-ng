//! `read_definition_scope_phase_entry` 表の DAO — この表 1 つの I/O だけを持つ。

use rusqlite::{Connection, Transaction};

use super::{DefinitionScopePhaseEntryRow, TableContent};
use crate::orchestration::{GlobalSeqNr, JournalReadError};

/// `read_definition_scope_phase_entry` 表 (スコープ × フェーズの入口ステージ) の DAO。
///
...
pub trait DefinitionScopePhaseEntryDao {
    /// 表と索引が無ければ作る (冪等)。
    ///
    /// # Errors
    ///
