//! `read_definition_scope_keyword` 表の DAO — この表 1 つの I/O だけを持つ。

use rusqlite::{Connection, Transaction};

use super::{DefinitionScopeKeywordRow, TableContent};
use crate::orchestration::{GlobalSeqNr, JournalReadError};

/// `read_definition_scope_keyword` 表 (スコープ推定の語 1 つ) の DAO。
///
...
pub trait DefinitionScopeKeywordDao {
    /// 表と索引が無ければ作る (冪等)。
    ///
    /// # Errors
    ///
