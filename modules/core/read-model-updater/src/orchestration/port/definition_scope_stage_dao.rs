//! `read_definition_scope_stage` 表の DAO — この表 1 つの I/O だけを持つ。

use rusqlite::{Connection, Transaction};

use super::{DefinitionScopeStageRow, TableContent};
use crate::orchestration::{GlobalSeqNr, JournalReadError};

/// `read_definition_scope_stage` 表 (スコープグリッドのセル 1 つ) の DAO。
///
...
pub trait DefinitionScopeStageDao {
    /// 表と索引が無ければ作る (冪等)。
    ///
    /// # Errors
    ///
