//! `read_definition` 表の DAO — この表 1 つの I/O だけを持つ。

use rusqlite::{Connection, Transaction};

use super::{DefinitionRow, TableContent};
use crate::orchestration::{GlobalSeqNr, JournalReadError};

/// `read_definition` 表 (定義 1 件の要約) の DAO。
///
...
pub trait DefinitionDao {
    ...
    /// `as_of` 列の最大値 (行が無ければ `None`)。
    ///
    /// 共有面がどの位置まで描かれていたかを、記録 (`amadeus_read_model_head`) を持たない
    /// 旧いストアで推し量る材料である (推し量るのは更新器)。
    ///
    /// # Errors
    ///
