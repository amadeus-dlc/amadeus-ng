//! `read_artifact_audit` 表の DAO — この表 1 つの I/O だけを持つ。

use rusqlite::{Connection, Transaction};

use super::{ArtifactAuditRow, TableContent};
use crate::orchestration::{GlobalSeqNr, JournalReadError};

/// `read_artifact_audit` 表 (成果物監査の対象ごとの最新観測) の DAO。
///
...
pub trait ArtifactAuditDao {
    ...
    /// `rows` を保存する (主キー `id` が同じ行は上書きする)。ただし保存済みの行のほうが
    /// 新しい断面 (`as_of` が大きい) なら、その行は上書きしない — 古い断面の書き直しで
    /// 新しい観測を巻き戻さない (1 表の中の書込の規則)。`as_of` は全行へ同じ値を書く。
    ///
    /// # Errors
    ///
