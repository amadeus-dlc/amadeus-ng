//! `read_next_jump_phase` 表の DAO — この表 1 つの I/O だけを持つ。

use rusqlite::{Connection, Transaction};

use super::{NextJumpPhaseRow, TableContent};
use crate::orchestration::{GlobalSeqNr, JournalReadError};

/// `read_next_jump_phase` 表 (フェーズ名で名指したジャンプ先) の DAO。
///
