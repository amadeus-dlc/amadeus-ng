//! `AnswerResultDao` の SQLite 実装 — `read_answer_result` 表 1 つだけを読み書きする。

use rusqlite::{Connection, Transaction, params};

use super::column_value::{count_schema_objects, read_content, scan_position};
use super::store_failure::SqliteResultExt;
use super::{AnswerResultDao, AnswerResultRow, GlobalSeqNr, JournalReadError, TableContent};

/// 表の DDL (冪等)。
///
/// 列の並びは内容の照合 ([`AnswerResultDao::find_content`]) の並びでもある。列を足す・落とす・型を
...
const TABLE_EXISTS: &str =
    "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'read_answer_result'";

/// 揃っているべき本数 (表 1 つと、その索引)。
const SCHEMA_OBJECTS: i64 = 1;

/// 表を落とす (索引も一緒に落ちる)。
const DROP_TABLE: &str = "DROP TABLE IF EXISTS read_answer_result";

/// 全行を消す (全差し替えの前半)。
const DELETE_ALL: &str = "DELETE FROM read_answer_result";

/// 行を足す。
const WRITE: &str = "INSERT INTO read_answer_result (id,answer_id,execution_id,stage,disposition,as_of) VALUES (?1,?1,?2,?3,?4,?5)";

/// 全行の生の値 (主キーの昇順)。
const SELECT_CONTENT: &str = "SELECT * FROM read_answer_result ORDER BY id";

/// `read_answer_result` 表の DAO の実装。状態を持たない。
