//! `SingleStageRunCommitted` の永続化 DTO (**読む側**)。

use core_command_domain::orchestration::SingleStageRunCommitted;
...
impl SingleStageRunCommittedDto {
    /// ドメインの公開アクセサだけを読んで DTO を組む (書き)。
    pub(super) fn of(payload: &SingleStageRunCommitted) -> SingleStageRunCommittedDto {
        SingleStageRunCommittedDto {
    ...
    /// ドメインの材料へ戻す (読み)。
    pub(super) fn to_domain(&self) -> Result<SingleStageRunCommitted, DtoDecodeError> {
        Ok(SingleStageRunCommitted::new(
            event_id_of(&self.id)?,
