//! `NextJumpPhaseDao` の SQLite 実装 — `read_next_jump_phase` 表 1 つだけを読み書きする。
...
use super::{GlobalSeqNr, JournalReadError, NextJumpPhaseDao, NextJumpPhaseRow, TableContent};

/// 表と索引の DDL (冪等)。
///
/// 列の並びは内容の照合 ([`NextJumpPhaseDao::find_content`]) の並びでもある。列を足す・落とす・型を
...
const TABLE_EXISTS: &str = "SELECT COUNT(*) FROM sqlite_master WHERE (type = 'table' AND name = 'read_next_jump_phase') OR (type = 'index' AND name IN ('read_next_jump_phase_key'))";

/// 揃っているべき本数 (表 1 つと、その索引)。
const SCHEMA_OBJECTS: i64 = 2;

/// 表を落とす (索引も一緒に落ちる)。
const DROP_TABLE: &str = "DROP TABLE IF EXISTS read_next_jump_phase";

/// 全行を消す (全差し替えの前半)。
const DELETE_ALL: &str = "DELETE FROM read_next_jump_phase";

/// 行を足す。
const WRITE: &str = "INSERT INTO read_next_jump_phase
             (id, execution_id, phase, target_index, target_slug, as_of)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6)";

/// 全行の生の値 (主キーの昇順)。
const SELECT_CONTENT: &str = "SELECT * FROM read_next_jump_phase ORDER BY id";

/// `read_next_jump_phase` 表の DAO の実装。状態を持たない。
#[derive(Debug, Clone, Copy, Default)]
pub struct NextJumpPhaseDaoImpl;

impl NextJumpPhaseDao for NextJumpPhaseDaoImpl {
