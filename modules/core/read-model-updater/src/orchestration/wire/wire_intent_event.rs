...
//!
//! 書く側 (command interface-adapter の `WireIntentEvent`) と**共有しない**同名の別の型で
//! ある (`coding-rules/cqrs-boundaries.md` — 側ごと専用化)。一致は横断適合テストが固定する。
//!
//! `Created` の中身は [`WireIntent`] そのもの — 誕生の材料 = 集約の全状態であり、復号結果は
...
impl WireIntentEvent {
    ...
    #[must_use]
    pub fn of(intent: &Intent) -> WireIntentEvent {
        WireIntentEvent::Created(WireIntent::of(intent))
    }
    ...
    pub fn to_domain(&self) -> Result<Intent, WireDecodeError> {
        match self {
            WireIntentEvent::Created(intent) => intent.to_domain(),
        }
    }
}
