//! `DefinitionScopeKeywordDao` の SQLite 実装 — `read_definition_scope_keyword` 表 1 つだけを読み書きする。
...
use super::{
    DefinitionScopeKeywordDao, DefinitionScopeKeywordRow, GlobalSeqNr, JournalReadError,
    TableContent,
};

/// 表と索引の DDL (冪等)。
///
/// 列の並びは内容の照合 ([`DefinitionScopeKeywordDao::find_content`]) の並びでもある。列を足す・落とす・型を
...
const TABLE_EXISTS: &str = "SELECT COUNT(*) FROM sqlite_master WHERE (type = 'table' AND name = 'read_definition_scope_keyword') OR (type = 'index' AND name IN ('read_definition_scope_keyword_key'))";

/// 揃っているべき本数 (表 1 つと、その索引)。
const SCHEMA_OBJECTS: i64 = 2;

/// 表を落とす (索引も一緒に落ちる)。
const DROP_TABLE: &str = "DROP TABLE IF EXISTS read_definition_scope_keyword";

/// 全行を消す (全差し替えの前半)。
const DELETE_ALL: &str = "DELETE FROM read_definition_scope_keyword";

/// 行を足す。
const WRITE: &str = "INSERT INTO read_definition_scope_keyword
             (id, definition_id, keyword, scope, as_of)
             VALUES (?1, ?2, ?3, ?4, ?5)";

/// 全行の生の値 (主キーの昇順)。
const SELECT_CONTENT: &str = "SELECT * FROM read_definition_scope_keyword ORDER BY id";

/// `read_definition_scope_keyword` 表の DAO の実装。状態を持たない。
#[derive(Debug, Clone, Copy, Default)]
pub struct DefinitionScopeKeywordDaoImpl;

impl DefinitionScopeKeywordDao for DefinitionScopeKeywordDaoImpl {
    ...
    fn insert(
        ...
        rows: &[DefinitionScopeKeywordRow],
        as_of: GlobalSeqNr,
