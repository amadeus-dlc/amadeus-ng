...
use core_command_domain::{
    orchestration::{ArtifactReuseError, IntentExecutionId, IntentId},
    workflow_definition::WorkflowDefinitionId,
};
/// 失敗した境界と原因を保持する。
#[derive(Debug)]
pub enum ArtifactReuseCommandError {
    /// 実行の取得・保存失敗。
    ...
    /// 集約による受領拒否。
    Rejected(ArtifactReuseError),
}
impl std::fmt::Display for ArtifactReuseCommandError {
...
impl std::error::Error for ArtifactReuseCommandError {}
