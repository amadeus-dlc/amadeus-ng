...
use super::{HookBinding, HookBindingDeclaration, ObservationFailure, WiredHook};

/// `settings.json` とその優先層から読んだフック配線の事実。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct HookWiring {
    settings_present: bool,
    wired_hooks: Result<Vec<WiredHook>, ObservationFailure>,
    hooks_disabled_by: Option<String>,
    managed_hooks_only: bool,
    bindings: Result<Vec<HookBinding>, ObservationFailure>,
...
impl HookWiring {
    ...
    pub const fn wired_hooks(&self) -> &Result<Vec<WiredHook>, ObservationFailure> {
        &self.wired_hooks
    }
    ...
    pub const fn bindings(&self) -> &Result<Vec<HookBinding>, ObservationFailure> {
        &self.bindings
    }
    ...
    /// このリポジトリ固有の接続定義 (無い作業ツリーでは [`HookBindingDeclaration::Absent`])。
    #[must_use]
