//! セッション監査集約の識別子。
use super::{HookHealthTarget, SessionAuditError};
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
/// 完成したセッション監査のドメイン型。
pub struct SessionAuditId(String);
impl SessionAuditId {
    ...
    pub fn parse(raw: &str) -> Result<Self, SessionAuditError> {
        let valid = raw.strip_prefix("session-audit:").is_some_and(|v| {
            v.len() == 64
                && v.bytes()
...
impl std::fmt::Display for SessionAuditId {
