//! `IntentEventId` — ドメインイベント [`IntentEvent`] 自身の識別子。
//!
//! [`IntentEvent`]: super::intent_event::IntentEvent

use std::fmt;

use uuid::Uuid;

use super::intent_event_id_error::IntentEventIdError;

/// intent のドメインイベント 1 件の識別子 (Always Valid — 不正値はこの型に存在しない)。
///
/// **ドメインイベントはエンティティの一種**なので自前の識別子を持つ (オーナー裁定
/// 2026-09-02、`coding-rules/domain-object-kinds.md`)。どの集約に起きた事実かは別の
/// フィールド `aggregate_id: IntentId` が運ぶ — 集約の ID をイベントの id に
/// 流用しない。
///
/// 形は [`IntentId`](super::intent_id::IntentId) と同じ
/// **UUIDv7 の正準表記**だが、**型が違えば取り違えはコンパイルで落ちる** (Entity + Id 法則)。
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct IntentEventId(String);
...
impl IntentEventId {
    ...
    pub fn parse(s: &str) -> Result<IntentEventId, IntentEventIdError> {
        let trimmed = s.trim();
        let Ok(uuid) = Uuid::try_parse(trimmed) else {
            return Err(IntentEventIdError::NotCanonicalUuidV7);
        };
        ...
        Ok(IntentEventId(trimmed.to_string()))
    }

    /// 新しい識別子を採番する (UUIDv7 — 時刻順に単調な乱数)。
    #[must_use]
    pub fn generate() -> IntentEventId {
        ...
        IntentEventId(Uuid::now_v7().as_hyphenated().to_string())
    }
...
impl fmt::Display for IntentEventId {
...
mod tests {
    ...
    fn parse_accepts_a_canonical_lowercase_uuidv7() {
        for raw in [
            ...
            let id = IntentEventId::parse(raw).unwrap();
    ...
    fn surrounding_whitespace_is_trimmed_before_validation() {
        let id = IntentEventId::parse("  0190aaaa-bbbb-7ccc-9ddd-eeeeffff0000\n").unwrap();
    ...
    fn a_spelling_outside_the_canonical_form_is_rejected() {
        for raw in [
            ...
            assert_eq!(
                IntentEventId::parse(raw),
                Err(IntentEventIdError::NotCanonicalUuidV7),
                "accepted {raw:?}"
            );
        }
    }
    ...
    fn generate_mints_a_fresh_canonical_identifier_every_time() {
        let minted: Vec<IntentEventId> = (0..16).map(|_| IntentEventId::generate()).collect();
        for id in &minted {
            // 採番した綴りは自分の parse を通る (Always Valid の閉性)。
            assert_eq!(IntentEventId::parse(id.as_str()).as_ref(), Ok(id));
        }
        let distinct: HashSet<&str> = minted.iter().map(IntentEventId::as_str).collect();
        assert_eq!(distinct.len(), minted.len(), "採番が重複した");
    }

    #[test]
    fn the_id_works_as_a_map_and_set_key() {
        let a = IntentEventId::parse(SAMPLE).unwrap();
        let b = IntentEventId::parse("  0190aaaa-bbbb-7ccc-9ddd-eeeeffff0000 ").unwrap();
    ...
    fn the_rejection_carries_material_not_wording() {
        assert_eq!(
            IntentEventIdError::NotCanonicalUuidV7.to_string(),
