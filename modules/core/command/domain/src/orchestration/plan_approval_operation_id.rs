//! 呼出側が発行し、再開時も同一と扱う承認操作の保存と結果取得を対応させる識別子。
use super::plan_approval_error::PlanApprovalError;
use std::fmt;
use uuid::Uuid;

/// 再開時も同一と扱う承認操作要求の識別子。イベントIDと実行IDは別の型で区別する。
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct PlanApprovalOperationId(String);
impl PlanApprovalOperationId {
    ...
    /// 呼出側が新しい再開時も同一と扱う承認操作を識別するために採番する。
    #[must_use]
...
impl fmt::Display for PlanApprovalOperationId {
