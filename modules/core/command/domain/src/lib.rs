...
//!
//! **禁じられるのは永続化の機構であって、AI-DLC の概念ではない**（同規則「概念と機構の
//! 線引き」）。監査イベントの分類・見出し語彙や `StorePath` のような `aidlc/` 配下の構造は
//! 方法論が定める概念なので、RMU がそれを**描く**からといって描かれる概念まで出ていく
//! わけではない — このクレートに残るのが正である。
