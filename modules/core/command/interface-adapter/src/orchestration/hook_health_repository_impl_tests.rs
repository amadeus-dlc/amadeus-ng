...
use core_command_domain::workspace::{HookHealthTarget, HookName, SpaceName};
fn at() -> DateTime<Utc> {
    "2026-09-09T01:00:00Z".parse().unwrap()
}
fn genesis(name: &str) -> (HookHealth, HookHealthEvent) {
    HookHealth::start(
...
async fn backend_contract<S>(mut repository: HookHealthRepositoryImpl<S>)
where
        ...
        + Clone,
{
    ...
    store
        .persist_event(
            EventEnvelope::new(key, 2, at(), HookHealthEventDto::of(&event))
    ...
    let event = aggregate
        .observe_heartbeat(at() + chrono::Duration::seconds(1))
        .unwrap();
    let payload = malformed(&HookHealthEventDto::of(&event), "kind");
    store
        .persist_event(
            EventEnvelope::new(
                HookHealthAggregateKeyDto::of(aggregate.id()),
                2,
                at(),
                payload,
            )
...
async fn memory_history_contract() {
    backend_contract(HookHealthRepositoryImpl::in_memory()).await;
}
#[tokio::test]
async fn sqlite_history_contract() {
    let temp = tempfile::tempdir().unwrap();
    let path = StorePath::for_runtime(temp.path());
    backend_contract(HookHealthRepositoryImpl::open(&path).unwrap()).await;
}
...
fn memory_errors_carry_no_dummy_location() {
    let repository = HookHealthRepositoryImpl::in_memory();
    assert!(matches!(
    ...
    assert!(matches!(
        HookHealthRepositoryImpl::<HookHealthMemoryStore>::io(
...
async fn snapshot_corruption_contract<S>(mut repository: HookHealthRepositoryImpl<S>)
where
    S: EventStore<AID = HookHealthAggregateKeyDto, A = HookHealthDto, P = HookHealthEventDto>
        + Clone,
{
    ...
    store
        .persist_event_and_snapshot(
            ...
            .with_manifest(MANIFEST),
            malformed(&HookHealthDto::of(&aggregate), "id"),
            0,
        )
        .await
        .unwrap();
    ...
    store
        .persist_event_and_snapshot(
            ...
            .with_manifest(MANIFEST),
            with_exhausted_sequence(&HookHealthDto::of(&aggregate)),
            0,
        )
        .await
        .unwrap();
...
async fn memory_snapshot_corruption_contract() {
    snapshot_corruption_contract(HookHealthRepositoryImpl::in_memory()).await;
}
...
async fn sqlite_snapshot_corruption_contract() {
    let temp = tempfile::tempdir().unwrap();
    let path = StorePath::for_runtime(temp.path());
    snapshot_corruption_contract(HookHealthRepositoryImpl::open(&path).unwrap()).await;
}
...
fn opening_a_missing_directory_is_reported_as_not_found() {
    ...
    assert!(matches!(
        HookHealthRepositoryImpl::open(&path),
        Err(RepositoryError::Io {
...
fn failure_vocabulary_contract<S>(
    repository: &HookHealthRepositoryImpl<S>,
    ...
    S: EventStore<AID = HookHealthAggregateKeyDto, A = HookHealthDto, P = HookHealthEventDto>,
{
    ...
    assert!(located(
        HookHealthRepositoryImpl::<S>::io(EventStoreWriteError::OtherError("other".into()), path),
        ErrorKind::Other
    ));
}
...
fn both_backends_share_the_failure_vocabulary() {
    let temp = tempfile::tempdir().unwrap();
    let path = StorePath::for_runtime(temp.path());
    failure_vocabulary_contract(&HookHealthRepositoryImpl::open(&path).unwrap(), Some(&path));
    failure_vocabulary_contract(
        &HookHealthRepositoryImpl::<HookHealthMemoryStore>::in_memory(),
        None,
    );
}
