...
impl PlanGenerationDto {
    pub(super) fn of(value: &PlanGeneration) -> Self {
        Self {
            id: value.id().to_string(),
            receipt: PlanReceiptDto::of(value.receipt()),
            state: match value.state() {
                PlanGenerationState::Pending => "pending",
                PlanGenerationState::Active => "active",
                PlanGenerationState::Revoked => "revoked",
            }
            .to_string(),
        }
    }
