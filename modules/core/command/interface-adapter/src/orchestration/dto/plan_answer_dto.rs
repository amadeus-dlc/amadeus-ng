...
impl PlanAnswerDto {
    pub(super) fn of(value: &PlanAnswer) -> Self {
        Self {
            id: value.id().to_string(),
            input: PlanAnswerInputDto::of(value.input()),
            occurrence_id: value.occurrence_id().to_string(),
            challenge_id: value.challenge_id().to_string(),
            response_id: value.response_id().to_string(),
            receipt: value.receipt().map(PlanReceiptDto::of),
            state: match value.state() {
                PlanAnswerState::Pending => "pending",
                PlanAnswerState::Recorded => "recorded",
                PlanAnswerState::Aborted(_) => "aborted",
            }
            .to_string(),
            error: match value.state() {
                PlanAnswerState::Aborted(error) => Some(error.clone()),
                _ => None,
            },
        }
    }
