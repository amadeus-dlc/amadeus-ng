//! `DtoDecodeError` — 行のバイトをドメインへ写せない理由 (材料のみ)。
...
impl std::fmt::Display for DtoDecodeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DtoDecodeError::Malformed { field, found } => {
                write!(f, "malformed field {field}: {found}")
            }
            DtoDecodeError::InvariantViolation => f.write_str("invariant violation"),
        }
    }
}
...
mod tests {
    ...
    fn the_rejection_renders_its_material() {
        assert_eq!(
            DtoDecodeError::malformed("id", "not-a-uuid").to_string(),
            "malformed field id: not-a-uuid"
        );
        assert_eq!(
            DtoDecodeError::InvariantViolation.to_string(),
            "invariant violation"
        );
    }
    ...
    fn the_rejection_is_a_std_error() {
        let error: Box<dyn std::error::Error> = Box::new(DtoDecodeError::InvariantViolation);
