...
//!
//! RMU (`core-read-model-updater`) にも同名の写像がある。**意図的な複製**である — 共有すれば
//! コマンド側が RMU を `Cargo.toml` に書くことになり、それは違反である
//! (`coding-rules/cqrs-boundaries.md`)。30 行の写像に側の独立を売り渡さない、という裁定
//! (構成案 §3)。
//!
//! 複製とはいえ、こちらは**コマンド側が実際に踏む経路**を持つ — 本家ストアが
//! `Box<dyn Error>` に包んで運ぶ失敗を開けて分類する。生の rusqlite を直接触るのは RMU
