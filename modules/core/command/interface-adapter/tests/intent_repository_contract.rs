...
type MemoryRepository = IntentRepositoryImpl<EventStoreForMemory<IntentId, Intent, IntentEvent>>;

/// SQLite ファイルのストアを内包した Repository。
type SqliteRepository = IntentRepositoryImpl<EventStoreForSqlite<IntentId, Intent, IntentEvent>>;
...
impl StoreFixture for SqliteFixture {
    ...
    fn reopen(&self, repository: &SqliteRepository) -> SqliteRepository {
        ...
        IntentRepositoryImpl::open(path).expect("同じファイルを開き直せる")
    }
}
