...
//!
//! 分類の正本をここ 1 つに保つのは、同じ SQLite の失敗を Repository 面 (本家の
//! `EventStoreWriteError` / `EventStoreReadError` 越し) と `JournalReader` 面 (rusqlite
//! 直接) の 2 か所で語るからである。綴りが割れると「再実行で解ける失敗」の見分けが面ごとに
//! 変わってしまう。
