...
use support::{absent_intent_id, at, contract, genesis, intent_id, store_and_reload};
...
/// genesis + 2 コマンドを書き、最後の集約 (握り直し済み) を返す。
async fn seed(repository: &mut Repository) -> WorkflowExecution {
    let mut aggregate = support::store_genesis(repository).await;
...
fn rewind_snapshot_to_genesis(conn: &Connection, payload: &[u8]) {
    conn.execute("UPDATE snapshot SET payload = ?1", [payload])
...
async fn the_contract_seed_writes_five_events() {
    let fixture = Fixture::new();
    let mut repository = fixture.repository();
    let aggregate = contract::seed(&mut repository).await;
