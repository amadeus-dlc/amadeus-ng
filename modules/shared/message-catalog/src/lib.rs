...
//! (`state::field_not_found_message` / `state::file_not_found_message` /
//! `lock::acquire_failed_message` / `bolt::invalid_mode_message`) はピン留めソースに対し
//! 4/4 バイト一致**することを確認した
...
pub mod state {
    ...
    #[must_use]
    pub fn field_not_found_message(field: &str) -> String {
        format!(
...
pub mod lock {
    ...
    /// エンベロープ経路で同一文言**という契約。
    #[must_use]
    pub const fn acquire_failed_message() -> &'static str {
    ...
    /// `withAuditLock` の acquire 失敗文言 — `acquire_failed_message()` とは**別系統**で、ロック
    ...
    #[must_use]
    pub fn acquire_failed_for_key_message(key: &str) -> String {
    ...
    #[must_use]
    pub fn merge_acquire_failed_message(lock_retries: u32, lock_retry_ms: u32) -> String {
...
pub mod bolt {
    ...
    #[must_use]
    pub fn invalid_mode_message(mode: &str) -> String {
...
mod tests {
    ...
    fn invalid_mode_message_is_verbatim() {
        assert_eq!(
            super::bolt::invalid_mode_message("turbo"),
    ...
    fn file_not_found_message_is_verbatim() {
        assert_eq!(
            super::state::file_not_found_message("/ws/aidlc-state.md"),
            "State file not found: /ws/aidlc-state.md"
        );
    }

    #[test]
    fn acquire_failed_message_is_verbatim() {
        assert_eq!(
            super::lock::acquire_failed_message(),
            "Failed to acquire audit lock after retries"
        );
    }

    #[test]
    fn acquire_failed_for_key_message_is_verbatim() {
        // aidlc-lib.ts:7599 — acquire_failed_message() とは別系統 (key を含む)
        assert_eq!(
            super::lock::acquire_failed_for_key_message("/ws\u{0}__workspace__"),
            "Failed to acquire audit lock for /ws\u{0}__workspace__ after retries"
        );
        assert_ne!(
            super::lock::acquire_failed_for_key_message("k"),
            super::lock::acquire_failed_message()
        );
    }

    #[test]
    fn merge_acquire_failed_message_is_verbatim_including_the_multiplication_sign() {
        // aidlc-audit.ts:1375 (既定値 50 × 100ms = 5.0s)
        assert_eq!(
            super::lock::merge_acquire_failed_message(50, 100),
            "Failed to acquire audit lock after 50 \u{00d7} 100ms = 5.0s retries; another merge in flight?"
        );
        assert!(
            super::lock::merge_acquire_failed_message(50, 100).contains(" \u{00d7} "),
            "区切りは U+00D7 (MULTIPLICATION SIGN) であり ASCII の 'x' ではない"
        );
    }

    #[test]
    fn merge_acquire_failed_message_rounds_seconds_like_js_to_fixed_1() {
        // 厳密なちょうど中間 (0.25 = 50 × 5ms) は JS `toFixed` の規約どおり切り上げ
        // (Rust の `{:.1}` の偶数丸めなら "0.2" になってしまう)。
        assert!(super::lock::merge_acquire_failed_message(50, 5).contains("= 0.3s retries"));
        assert!(super::lock::merge_acquire_failed_message(3, 250).contains("= 0.8s retries")); // 0.75
        // 二進で厳密に表せない 0.15 は double が 0.15 未満なので切り捨て (JS と同じ)。
        assert!(super::lock::merge_acquire_failed_message(3, 50).contains("= 0.1s retries"));
        assert!(super::lock::merge_acquire_failed_message(0, 100).contains("= 0.0s retries"));
    }

    #[test]
    fn field_not_found_message_is_verbatim() {
        assert_eq!(
            super::state::field_not_found_message("Construction Autonomy Mode"),
