...
**I/Oの例外（2026-09-14）**: コマンド側のRepository traitが宣言する `find_by_id` に、引数型と一致するドメインの `XxxId` を直接渡すgetterだけを許可する。借用・括弧・IDのclone・UFCSも同じ扱いとする。

```rust
// 許可: 参照先のIDで取得する。
let definition = repository.find_by_id(intent.definition_id()).await?;
```

IDの比較・分岐・内部表現への変換、ID以外の状態取得は許可しない。ID取得のローカル変数・補助関数・ブロックへの移動も例外対象にしない。引数に含まれる式全体を検査から外さず、ID取得の受信者にある別のgetterも検査する。同名の通常型のメソッドをRepositoryとみなさず、trait宣言・引数型・getterの戻り値型を照合する。
...
これは rustc の型検査器ではない。マクロ展開・関数ポインタ・動的 dispatch・関連型・外部 crate の契約・複雑な制御フローや型パターンは復元しない。glob による再公開、Cargo で別名を付けた依存や独自の lib 名、`#[path]` によるモジュール配置も解決しない。`<T as Trait>::method` は索引に存在するtrait契約へ解決するが、任意のtrait実装の選択までは行わない。crate 名は `modules/` から `src/` までの配置（区切りとハイフンをアンダースコアへ置換）に対応する本リポジトリの規約を使用する。
...
IDを読む処理はinterface-adapter側に置いた。これは導入時の対応であり、2026-09-14の裁定で型付きIDの直接受け渡しを許可し、参照元オブジェクトを検索引数に取るメソッドは撤去した。
